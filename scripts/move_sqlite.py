...
def main():
    ...
    if not os.path.exists(args.f):
        print(
            f"Warning: '{args.f}' does not exist (yet). Is the path specified correctly?"
        )
        return
    ...
    print(
        f"{num} new entries in backup database found ({num_ranks} ranks, {num_teamranks} teamranks, {num_saves} saves"
    )
    ...
    print(
        (
            f"    echo '.dump --preserve-rowids' | sqlite3 {os.path.abspath(args.to)} | "
            + "grep -E '^INSERT INTO record_(race|teamrace|saves)' | "  # including rowids, this forces sqlite to name all columns in each INSERT statement
            + "sed -e 's/INSERT INTO/INSERT IGNORE INTO/' | "  # filter out inserts
            + f"sed -e 's/rowid,//' -e 's/VALUES([0-9]*,/VALUES(/' > {sql_file}"  # ignore duplicate rows
        )
    )  # filter out rowids again
    print(f"    mysql -u teeworlds -p'PW2' teeworlds < {sql_file}")
    print()
    print(
        f"When the ranks are transfered successfully to mysql, {os.path.abspath(args.to)} can be removed"
    )
    print()
