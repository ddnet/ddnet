...
AUTH_ADD_REGEX = re.compile(
    r'^\s*auth_add\s+(?P<username>"[^"]*"|[^"\s]+)\s+(?P<level>"[^"]*"|[^"\s]+)\s+(?P<password>"[^"]*"|[^"\s]+)\s*$'
)
...
def main():
    p = argparse.ArgumentParser(
        description="Hash passwords in a way suitable for DDNet configs."
    )
    p.add_argument(
        "--new",
        "-n",
    ...
    if use_stdio:
        ...
        input_file = open(
            args.config, encoding="utf-8"
        )  # pylint: disable=consider-using-with
        output_file = tempfile.NamedTemporaryFile(
            "w", dir=os.getcwd(), prefix=f"{args.config}.", delete=False
        )  # pylint: disable=consider-using-with
