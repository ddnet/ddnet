...
class Record(namedtuple("Record", "name time checkpoints")):
    ...
    def parse(lines):
        ...
        if len(checkpoints_str) != 26 or checkpoints_str[25] != "":
            raise ValueError(
                f"wrong amount of checkpoint times: {len(checkpoints_str)}"
            )
...
def main():
    ...
    p.add_argument(
        "--out", default="ddnet-server.sqlite", help="Output SQLite database"
    )
    p.add_argument(
        "in_",
        metavar="IN",
    ...
    for in_ in args.in_:
        ...
        if not match:
            raise ValueError(
                f"Invalid text score database name, does not end in '_record.dtb': {in_}"
            )
        m = match.group("map")
        if m in records:
