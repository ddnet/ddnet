...
# generate keynames_readable.cpp file
with open("src/engine/client/keynames_readable.cpp", "w", encoding="utf-8") as f:
	print("/* AUTO GENERATED! DO NOT EDIT MANUALLY! See scripts/gen_keys.py */", file=f)
	print(file=f)
	print('#include "keynames_readable.h"', file=f)
	print(file=f)
	print('#include "keyboard.h"', file=f)
	print(file=f)
	print("/**", file=f)
	print(" * Do not use directly! Use the @link KeyNameHumanReadable @endlink function.", file=f)
	print(" */", file=f)
	print("char (*KeyNamesHumanReadable())[20]", file=f)
	print("{", file=f)
	print("\tstatic char s_aaGeneratedKeyNames[512][20];", file=f)
	print("\tstatic bool s_Generated = [] {", file=f)
	print(f"\t\tfor(int i = 0; i < {len(keynames)}; ++i)", file=f)
	print("\t\t\tGenerateKeyNameHumanReadable(s_aaGeneratedKeyNames[i], i);", file=f)
	print("\t\treturn true;", file=f)
	print("\t}();", file=f)
	print("\t(void)s_Generated;", file=f)
	print("\treturn s_aaGeneratedKeyNames;", file=f)
	print("}", file=f)

# generate keynames_readable.h file
with open("src/engine/client/keynames_readable.h", "w", encoding="utf-8") as f:
	print("/* AUTO GENERATED! DO NOT EDIT MANUALLY! See scripts/gen_keys.py */", file=f)
	print(file=f)
	print("#ifndef ENGINE_CLIENT_KEYNAMES_READABLE_H", file=f)
	print("#define ENGINE_CLIENT_KEYNAMES_READABLE_H", file=f)
	print(file=f)
	print("/**", file=f)
	print(" * Do not use directly! Use the @link KeyNameHumanReadable @endlink function.", file=f)
	print(" */", file=f)
