...
def main():
    ...
    cmds = split_cmds(
        subprocess.check_output([otool, "-l", executable]).decode().splitlines()
    )
    ...
    rpaths = {
        k[0]
        for k in [
            [path_regex.match(part).group(1) for part in cmd if path_regex.match(part)]
            for cmd in lc_rpath_cmds
        ]
    }
    print("Found paths:")
