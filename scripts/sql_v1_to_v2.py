...
class IdCache:
	...
	def get(self, name, insert_cur=None):
		...
		cur.execute(f"INSERT INTO {self.table}(name) VALUES (%s) ON CONFLICT DO NOTHING", (name,))
...
def migrate_maps(src, pg, prefix):
	...
	for row in iter_rows(src, f"SELECT Map, Server, Mapper, Points, Stars, Timestamp FROM {prefix}_maps"):
		...
		cur.execute(f"INSERT INTO {prefix}_map(name, category, mapper, points, stars, released) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (name) DO UPDATE SET category = EXCLUDED.category, mapper = EXCLUDED.mapper, points = EXCLUDED.points, stars = EXCLUDED.stars, released = EXCLUDED.released", row)
		n += 1
	pg.commit()
	log(f"maps: {n}")
...
def migrate_players(src, pg, prefix):
	...
	cur = pg.cursor()
	n = 0
	for query in (f"SELECT DISTINCT Name FROM {prefix}_race", f"SELECT DISTINCT Name FROM {prefix}_teamrace", f"SELECT DISTINCT Name FROM {prefix}_points"):
		buf = []
		for (name,) in iter_rows(src, query):
			buf.append((clean_name(name),))
			if len(buf) >= BATCH:
				psycopg2.extras.execute_values(cur, f"INSERT INTO {prefix}_player(name) VALUES %s ON CONFLICT DO NOTHING", buf)
				n += len(buf)
				buf = []
		if buf:
			psycopg2.extras.execute_values(cur, f"INSERT INTO {prefix}_player(name) VALUES %s ON CONFLICT DO NOTHING", buf)
...
def migrate_finishes(src, pg, prefix, since):
	...
	query = f"SELECT Map, Name, Timestamp, Time, Server, {cp_cols}, GameId, DDNet7 FROM {prefix}_race {where}"
	...
	def flush(batch):
		...
		if since:
			...
			psycopg2.extras.execute_values(cur, f"INSERT INTO {prefix}_finish(map_id, player_id, time_cs, finished_at, server, game_uuid, cp_times, ddnet7) VALUES %s ON CONFLICT DO NOTHING", batch)
		else:
			buf = io.StringIO()
			for row in batch:
				buf.write(row)
			buf.seek(0)
			cur.copy_expert(f"COPY {prefix}_finish(map_id, player_id, time_cs, finished_at, server, game_uuid, cp_times, ddnet7) FROM STDIN", buf)
		pg.commit()
	...
	for row in iter_rows(src, query, args):
		...
		if since:
			...
			batch.append((maps.ids[map_name], players.ids[name], time_to_cs(time), clean_ts(timestamp), server if server is not None else "", clean_uuid(game_uuid), pack_cp_times(cps), bool(ddnet7)))
		else:
...
def migrate_teams(src, pg, prefix, since):
	...
	def flush_team(team_id, map_name, names, time, timestamp, game_uuid, ddnet7):
		...
		cur.execute(
			...
			(psycopg2.Binary(team_id), map_id, psycopg2.Binary(h), time_to_cs(time), clean_ts(timestamp), clean_uuid(game_uuid), len(names), bool(ddnet7)),
		)
		kept_id = bytes(cur.fetchone()[0])
		# same roster -> same members, only insert them for the kept team row
		psycopg2.extras.execute_values(cur, f"INSERT INTO {prefix}_team_player(team_id, player_id) VALUES %s ON CONFLICT DO NOTHING", [(psycopg2.Binary(kept_id), player_id) for player_id in member_ids])
...
def migrate_saves(src, pg, prefix):
	...
	cur = pg.cursor()
	n = 0
	for map_name, code, savegame, timestamp, server, save_uuid, ddnet7 in iter_rows(src, f"SELECT Map, Code, Savegame, Timestamp, Server, SaveId, DDNet7 FROM {prefix}_saves"):
		map_id = maps.get(map_name, cur)
		cur.execute(f"INSERT INTO {prefix}_save(map_id, code, savegame, saved_at, server, save_uuid, ddnet7) VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (map_id, code) DO NOTHING", (map_id, code, savegame, clean_ts(timestamp), server, clean_uuid(save_uuid), bool(ddnet7)))
		n += 1
	pg.commit()
	log(f"saves: {n}")
...
def migrate_points(src, pg, prefix):
	...
	def flush():
		psycopg2.extras.execute_values(cur, f"INSERT INTO {prefix}_player_points(player_id, points) VALUES %s ON CONFLICT (player_id) DO UPDATE SET points = EXCLUDED.points", batch)
...
def cmd_finalize(args):
	...
	cur.execute("SELECT COUNT(*) FROM pg_constraint WHERE conname = %s", (f"{p}_finish_pkey",))
	if cur.fetchone()[0] == 0:
		try:
			...
			cur.execute(f"DELETE FROM {p}_finish a USING {p}_finish b WHERE a.ctid > b.ctid AND a.map_id = b.map_id AND a.player_id = b.player_id AND a.time_cs = b.time_cs AND a.finished_at = b.finished_at AND a.server = b.server")
	...
	# time-ordered walk for the NOT EXISTS top query
	cur.execute(f"CREATE INDEX IF NOT EXISTS {p}_best_map_time ON {p}_best(map_id, time_cs)")
...
def cmd_replay(args):
	...
	for table in (f"{p}_finish", f"{p}_finish_backup"):
		...
		for map_name, name, time_cs, finished_at, server, game_uuid, cp_times, ddnet7 in rows:
			...
			if args.award_points:
				cur.execute(f"SELECT COUNT(*) FROM {p}_finish WHERE map_id = %s AND player_id = %s", (map_id, player_id))
				first = cur.fetchone()[0] == 0
			cur.execute(
				f"INSERT INTO {p}_finish(map_id, player_id, time_cs, finished_at, server, game_uuid, cp_times, ddnet7) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
				(map_id, player_id, time_cs, finished_at, server or "", clean_uuid(game_uuid), psycopg2.Binary(cp_times) if cp_times is not None else None, bool(ddnet7)),
			)
			if cur.rowcount == 0:
			...
			if first:
				...
				if map_points:
					cur.execute(f"INSERT INTO {p}_player_points(player_id, points) VALUES (%s, 0) ON CONFLICT DO NOTHING", (player_id,))
					cur.execute(f"UPDATE {p}_player_points SET points = points + %s WHERE player_id = %s", (map_points, player_id))
			...
			cur.execute(
				...
				(map_id, player_id, server or "", time_cs, cp_time, psycopg2.Binary(cp_times) if cp_times is not None else None, finished_at, finished_at),
			)
			cur.execute(f"UPDATE {p}_player SET first_finished = LEAST(COALESCE(first_finished, %s), %s) WHERE player_id = %s", (finished_at, finished_at, player_id))
	...
	for table in (f"{p}_team", f"{p}_team_backup"):
		...
		for team_id, map_name, time_cs, finished_at, server, game_uuid, member_count, ddnet7, names in rows:
			...
			cur.execute(
				...
				(psycopg2.Binary(bytes(team_id)), map_id, psycopg2.Binary(h), time_cs, finished_at, server or "", clean_uuid(game_uuid), member_count, bool(ddnet7)),
			)
			kept_id = bytes(cur.fetchone()[0])
			psycopg2.extras.execute_values(cur, f"INSERT INTO {p}_team_player(team_id, player_id) VALUES %s ON CONFLICT DO NOTHING", [(psycopg2.Binary(kept_id), player_id) for player_id in member_ids])
			teams += 1
	...
	for table in (f"{p}_save", f"{p}_save_backup"):
		...
		for map_name, code, savegame, saved_at, server, save_uuid, ddnet7 in rows:
			map_id = maps.get(map_name, cur)
			cur.execute(f"INSERT INTO {p}_save(map_id, code, savegame, saved_at, server, save_uuid, ddnet7) VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (map_id, code) DO NOTHING", (map_id, code, savegame, saved_at, server, clean_uuid(save_uuid), bool(ddnet7)))
			saves += cur.rowcount
...
def cmd_verify(args):
	...
	check("finish count", one(src, f"SELECT COUNT(*) FROM {p}_race")[0], one(pg, f"SELECT COUNT(*) FROM {p}_finish")[0])
	check(
		"time checksum",
		one(src, f"SELECT COALESCE(SUM(LEAST(CAST(ROUND(Time * 100) AS SIGNED), 2147483647)), 0) FROM {p}_race" if src[0] == "mysql" else f"SELECT COALESCE(SUM(MIN(CAST(ROUND(Time * 100) AS INTEGER), 2147483647)), 0) FROM {p}_race")[0],
	...
	rows = list(iter_rows(src, f"SELECT Map, MIN(Time), COUNT(*) FROM {p}_race GROUP BY Map ORDER BY COUNT(*) DESC LIMIT 20"))
	for map_name, best, count in rows:
		v2 = one(
			pg,
			f"""
            SELECT MIN(b.time_cs), COALESCE(SUM(b.finish_count), 0) FROM {p}_best b
            JOIN {p}_map m ON m.map_id = b.map_id WHERE m.name = %s""",
			(map_name,),
		)
...
def main():
	...
	def add(name, func, source=False):
		...
		if source:
			cmd.add_argument("--from", dest="source", required=True, help="mysql://user:pass@host:port/db or sqlite:///path")
		return cmd

	add("create-schema", cmd_create_schema)
	migrate = add("migrate", cmd_migrate, source=True)
	migrate.add_argument("--since", default=None, help="delta mode: only copy finishes/teams with Timestamp > SINCE (UTC); requires a prior `finalize`")
	finalize = add("finalize", cmd_finalize)
	finalize.add_argument("--team-servers", action="store_true", help="derive team.server from matching finishes (for regional team ranks, slow)")
	replay = add("replay", cmd_replay, source=True)
	replay.add_argument("--award-points", action="store_true", help="award map points for first finishes, like the game's SaveScore")
