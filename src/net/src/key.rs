...
impl Identity {
    ...
    pub fn try_from_lib<T: boring::pkey::HasPublic>(key: &boring::pkey::PKeyRef<T>)
        -> Option<Identity>
    {
        ...
        unsafe {
            ...
            if boring_sys::EVP_PKEY_get_raw_public_key(key.as_ptr(), buf.as_mut_ptr(), &mut len) != 1 {
                return None;
            }
            if len != buf.len() {
                return None;
...
fn random_bignum(bits: i32) -> boring::bn::BigNum {
    ...
    result.rand(bits, boring::bn::MsbOption::MAYBE_ZERO, false).unwrap();
    result
}
...
impl PrivateIdentity {
    ...
    pub fn generate_certificate(&self) -> boring::x509::X509 {
        let name = {
            ...
            builder.append_entry_by_nid(boring::nid::Nid::ORGANIZATIONNAME, "ddnet16-autogen").unwrap();
            builder.append_entry_by_nid(boring::nid::Nid::COMMONNAME, &format!("{}", self.public())).unwrap();
            builder.append_entry_by_nid(boring::nid::Nid::COMMONNAME, "a").unwrap();
            builder.build()
        };
        ...
        builder.set_serial_number(&random_bignum(160).to_asn1_integer().unwrap()).unwrap();
        builder.set_issuer_name(&name).unwrap();
        builder.set_not_before(&boring::asn1::Asn1Time::days_from_now(0).unwrap()).unwrap();
        builder.set_not_after(&boring::asn1::Asn1Time::days_from_now(7).unwrap()).unwrap();
        ...
        builder.append_extension(boring::x509::extension::BasicConstraints::new().critical().ca().build().unwrap()).unwrap();
...
mod test {
    ...
    fn generate_certificate() {
        let private_identity: PrivateIdentity = "89b84bbc4b430a74642a8d6ee9086048318b20090e5a5d0c807aba4ce2c0d22f".parse().unwrap();
