...
pub fn client2_main() -> Result<()> {
    env_logger::init();
    let identity = "6a35f116046280da62602ad979e151daee8e9109befd34337427bbb968c2bcbc".parse().unwrap();
    ...
    loop {
        while let Some((idx, event)) = net.recv(&mut buf).context("Net::recv")? {
            ...
            match event {
                ...
                Chunk(size, nonvital) => {
                    println!("{}: {}{:?}", idx, if nonvital { "nonvital " } else { "" }, BStr::new(&buf[..size]));
                },
                Disconnect(size, code) => {
                    println!("{}: disconnect ({}, {:?})", idx, code, BStr::new(&buf[..size]));
                    return Ok(());
                }
            }
        }
...
pub fn server2_main() -> Result<()> {
    ...
    let identity = "5cf2a4f0ed3dc85b3f4bfa5ca97b8adeaf0d5ec165179af805a3f7751dd8ac47".parse().unwrap();
    let mut builder = Net::builder();
    builder.identity(identity);
    ...
    loop {
        while let Some((idx, event)) = net.recv(&mut buf).context("Net::recv")? {
            ...
            match event {
                ...
                Chunk(size, nonvital) => {
                    println!("{}: recv {}{:?}", idx, if nonvital { "nonvital " } else { "" }, BStr::new(&buf[..size]));
                    if !nonvital {
                        net.close(idx, Some("blib?")).unwrap();
                    }
                },
                Disconnect(size, code) => println!("{}: disconnect ({}, {:?})", idx, code, BStr::new(&buf[..size])),
            }
        }
        net.wait();
    }
