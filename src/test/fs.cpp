...
TEST(Filesystem, CreateDeleteDirectory)
{
	...
	EXPECT_TRUE(fs_make_dir(Info.m_aFilename));
	IOHANDLE File = io_open(aFilename, IOFLAG_WRITE);
	ASSERT_TRUE(File);
	EXPECT_FALSE(io_close(File));

	// Directory removal fails if there are any files left in the directory.
	EXPECT_FALSE(fs_remove_dir(Info.m_aFilename));

	EXPECT_TRUE(fs_remove(aFilename));
	EXPECT_TRUE(fs_remove_dir(Info.m_aFilename));
}
...
TEST(Filesystem, CantDeleteDirectoryWithRemove)
{
	CTestInfo Info;
	EXPECT_TRUE(fs_make_dir(Info.m_aFilename));
	EXPECT_FALSE(fs_remove(Info.m_aFilename)); // Cannot remove directory with file removal function.
	EXPECT_TRUE(fs_remove_dir(Info.m_aFilename));
}
...
TEST(Filesystem, CantDeleteFileWithRemoveDirectory)
{
	...
	ASSERT_TRUE(File);
	EXPECT_FALSE(io_close(File));
	EXPECT_FALSE(fs_remove_dir(Info.m_aFilename)); // Cannot remove file with directory removal function.
	EXPECT_TRUE(fs_remove(Info.m_aFilename));
}
