...
TEST_F(CTestGameWorld, CharacterEmote)
{
	...
	// afk
	pPlayer->SetAfk(true);
	ASSERT_EQ(pChr->CurrentEyeEmote(), EMOTE_BLINK);

	// not afk
	pPlayer->SetAfk(false);
	ASSERT_EQ(pChr->CurrentEyeEmote(), EMOTE_NORMAL);
