...
TEST(RconRole, DisallowCommandIsCaseInsensitive)
{
	CAuthManager Manager;
	CRconRole *pMod = Manager.FindRole(RoleName::MODERATOR);
	// console command lookup is case insensitive
...
TEST(RconRolePeer, SameRoleCanKick)
{
	CAuthManager Manager;
	CRconRole *pMod = Manager.FindRole(RoleName::MODERATOR);
	CRconRole *pHelper = Manager.FindRole(RoleName::HELPER);
	EXPECT_TRUE(pMod->CanKick(pMod)) << "moderator can no longer kick another moderator";
	EXPECT_TRUE(pHelper->CanKick(pHelper)) << "helper can no longer kick another helper";
}
