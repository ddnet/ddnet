...
#define MACRO_CONFIG_FLOAT(Name, ScriptName, Def, Min, Max, Flags, Desc) \
	static constexpr CFixedPointNumber ms_##Name = Def; \
	float m_##Name;
...
struct SFloatConfigVariable : public SConfigVariable
{
	...
	SFloatConfigVariable(IConsole *pConsole, const char *pScriptName, EVariableType Type, int Flags, const char *pHelp, CFixedPointNumber *pVariable, float Default, float Min, float Max) :
		SConfigVariable(pConsole, pScriptName, Type, Flags, pHelp),
		m_pVariable(pVariable),
		m_Default(Default),
		m_Min(Min),
