 ...
 *
 * 2.
 *      The load failed callback takes 3 parameters and returns a bool. Its first parameter is a LOAD_ERROR which describes the nature
 *      of the error and what the value of the second parameter (user data) will be. The second parameter can be cast into the type
 *      specified in the LOAD_ERROR value to get error-specific data. The third parameter is user data. The error callback's return
 *      value is used to determine whether or not to proceed in the file load process after a potentially non-fatal issue has been encountered.
 *
 ...
 *
 *      The file loaded callback takes 4 parameters and has no return value. Its first parameter is the name of the file loaded, the
 *      second is a pointer to the file data, the third is the size of the data in bytes, and the fourth is user data. This data must be
 *      copied if it's meant to be retained; the pointer cannot be held after the callback is returned from. If the flags contain
 *      LOAD_FLAGS_DONT_READ_FILE, the second and third parameters will both be null.
 *
 *      You can set it by calling SetFileLoadedCallback(std::function<FileLoadedCallbackSignature> Function), where
 *      FileLoadedCallbackSignature is void(const std::string &ItemName, const unsigned char *pData, const unsigned int Size, void *pUser).
 *
 * 3.
...
public:
	...
	enum ELoadFlags : uint8_t
	{
		...
		LOAD_FLAGS_ABSOLUTE_PATH =		0b00000010,	// Return an absolute file path instead of just the filename. This will influence the regex.
		...
		LOAD_FLAGS_DONT_READ_FILE =		0b00000100,	// Do not read file contents.
		...
		LOAD_FLAGS_SKIP_BOM =			0b00001000,	// Whether or not to use IOFLAG_SKIP_BOM (string?)
		LOAD_FLAGS_ASYNC =			0b00010000,	// Load asynchronously instead; return value of Begin() is not used and a file load finished callback is fired instead
		// clang-format on
	};
...
protected:
	...
	inline static bool CompareExtension(const std::filesystem::path &Filename, std::string_view Extension);
