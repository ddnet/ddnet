...
static bool push_event(context_data *ctx_data, int type, const NETADDR *addr, const void *data, size_t size)
{
...
static void session_close(context_data *ctx_data, per_session_data *pss)
{
	...
	if(pss->notify_close && !push_event(ctx_data, WEBSOCKET_EVENT_CLOSE, &pss->addr, pss->close_reason, str_length(pss->close_reason) + 1))
	{
...
int websocket_recv_event(int socket, websocket_event *event)
{
	...
	else if(chunk->type == WEBSOCKET_EVENT_CLOSE && chunk->size > 0)
	{
...
int websocket_send(int socket, const unsigned char *data, size_t size, const NETADDR *addr)
{
	...
	if(chunk == nullptr)
	{
		// The send buffer is full, so the transport is stalled. Unlike UDP
		// there is no retransmission, so dropping the chunk would silently and
		// permanently break the reliable stream; close the connection instead.
		// The engine did not ask for this close, so it gets the close event.
...
void websocket_close(int socket, const NETADDR *addr, const char *reason)
{
	...
	for(int i = 0; i < 10 && ctx_data->port_map.count(*addr) > 0; i++)
	{
		lws_service(context, -1);
	}
}
