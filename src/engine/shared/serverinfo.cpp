...
{
	const char *apSkinPartNames[protocol7::NUM_SKINPARTS] = {"body", "marking", "decoration", "hands", "feet", "eyes"};
	...
	for(unsigned i = 0; i < Clients.u.array.length; i++)
	{
		...
		if(i < SERVERINFO_MAX_CLIENTS)
		{
			...
			if(SkinObj.type == json_object)
			{
				...
				// 0.7 skin
				else if(SkinObj[apSkinPartNames[protocol7::SKINPART_BODY]].type == json_object)
				{
					for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
					{
						const json_value &SkinPartObj = SkinObj[apSkinPartNames[Part]];
						if(SkinPartObj.type == json_object)
						{
							...
							if(SkinPartName.type == json_string)
							{
								...
								if(Part == protocol7::SKINPART_BODY && pClient->m_aaSkin7[Part][0] == '\0')
