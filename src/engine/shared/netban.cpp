...
void CNetBan::Init(IConsole *pConsole, IStorage *pStorage)
{
	...
	Console()->Register("bans", "?i[page]", CFGFLAG_SERVER | CMDFLAG_RESPONSE, ConBans, this, "Show banlist (page 1 by default, 20 entries per page)");
	Console()->Register("bans_find", "s[ip]", CFGFLAG_SERVER | CMDFLAG_RESPONSE, ConBansFind, this, "Find all ban records for the specified IP address");
