...
void CNetConnection::WebsocketOnClose(const NETADDR *pAddr, const char *pReason)
{
	...
	if(g_Config.m_Debug)
		dbg_msg("conn", "websocket closed reason='%s'", aStr);
}
...
int CNetConnection::QueueChunkEx(int Flags, int DataSize, const void *pData, int Sequence)
{
	...
	if(IsWebsocket())
	{
		...
		if(websocket_send(net_socket_websocket(m_Socket, m_PeerAddr.type), aMessage, 1 + DataSize, &m_PeerAddr) < 0)
			return -1;
		m_LastSendTime = time_get();
		return 0;
	}
	...
	if(State() == EState::ONLINE)
	{
		...
		if(time_get() - m_LastSendTime > time_freq())
		{
			if(IsWebsocket())
			{
				...
				websocket_send(net_socket_websocket(m_Socket, m_PeerAddr.type), nullptr, 0, &m_PeerAddr);
				m_LastSendTime = time_get();
			}
			else
			{
