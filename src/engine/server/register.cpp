...
void CRegister::CProtocol::CheckChallengeStatus()
{
	...
	{
		if(m_pShared->m_LatestResponseStatus == STATUS_OK && m_pShared->m_ConsecutiveFailures == 0)
		{
			// The last registration succeeded, so don't let backoff from
			// earlier failed attempts delay the next register.
			m_NextRegister = std::min(m_NextRegister, m_PrevRegister + 15 * time_freq());
		}
