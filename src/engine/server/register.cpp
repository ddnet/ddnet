	...
	}
{
	...
	m_pConsole->Chain("sv_community_token", ConchainOnConfigChange, this);
}
...
void CRegister::OnConfigChange()
{
	...
	m_GotCommunityToken = (bool)m_pConfig->m_SvCommunityToken[0];
	if(m_GotCommunityToken)
	{
		str_copy(m_aCommunityToken, m_pConfig->m_SvCommunityToken);
	}
	m_NumExtraHeaders = 0;
