...
bool CRconRole::CanKick(CRconRole *pRole) const
{
	...
	if(pRole->IsAdmin())
		return false;
	// equal roles should be able to kick each other
	if(pRole == this)
		return true;
...
void CRconRole::LogRconCommandAccess(int MaxLineLength)
{
	if(IsAdmin())
	{
		log_info("server", "The admin role has access to all commands and configs");
		return;
	}
...
void CAuthManager::SetTeleOthersConfig(const char *pRoles)
{
	...
	while((pNextRole = str_next_token(pNextRole, ",", aRole, sizeof(aRole))))
	{
		...
		if(!pRole)
		{
			if(!str_comp(aRole, "1"))
			{
				log_warn("server", "got deprecated value %s for sv_tele_others_auth_level, please use \"helper\" instead", aRole);
				pRole = FindRole(RoleName::HELPER);
			}
			else if(!str_comp(aRole, "2"))
			{
				log_warn("server", "got deprecated value %s for sv_tele_others_auth_level, please use \"moderator\" instead", aRole);
				pRole = FindRole(RoleName::MODERATOR);
			}
			else if(!str_comp(aRole, "3"))
			{
				log_warn("server", "got deprecated value %s for sv_tele_others_auth_level, please use \"admin\" instead", aRole);
				pRole = FindRole(RoleName::ADMIN);
			}
		}
		if(!pRole)
