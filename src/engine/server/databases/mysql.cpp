...
const char *CMysqlConnection::MedianMapTime(char *pBuffer, int BufferSize) const
{
	...
	str_format(pBuffer, BufferSize,
		...
		"      WHEN 1 THEN CASE WHEN rn = counter DIV 2 + 1 THEN Time END "
		"    END "
		"  ) OVER (PARTITION BY Map) AS Median "
		"FROM ("
		"  SELECT Map, Time, "
		"    ROW_NUMBER() OVER (PARTITION BY Map ORDER BY Time) AS rn, "
		"    COUNT(*) OVER (PARTITION BY Map) AS counter "
		"  FROM %s_race WHERE Map = l.Map"
		") AS r "
		"LIMIT 1",
		GetPrefix());
	return pBuffer;
