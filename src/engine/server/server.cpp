...
int CServer::NewClientNoAuthCallback(int ClientId, void *pUser)
{
	...
	pThis->m_aClients[ClientId].m_GotDDNetVersionPacket = false;
	pThis->m_aClients[ClientId].m_DDNetVersionSettled = false;
	pThis->m_aClients[ClientId].m_ClientsDummy = -1;
	pThis->m_aClients[ClientId].Reset();
...
int CServer::DelClientCallback(int ClientId, const char *pReason, void *pUser)
{
	...
	if(pThis->m_aClients[ClientId].m_ClientsDummy != -1)
	{
		pThis->m_aClients[pThis->m_aClients[ClientId].m_ClientsDummy].m_ClientsDummy = -1;
		pThis->m_aClients[ClientId].m_ClientsDummy = -1;
	}
			...
			if((pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && m_aClients[ClientId].m_State == CClient::STATE_PREAUTH)
			{
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					...
					if(m_aClients[ClientId].m_ConnectionId != m_aClients[i].m_ConnectionId ||
						net_addr_comp_noport(&pPacket->m_Address, m_NetServer.ClientAddr(i)) != 0)
						continue;

					m_aClients[ClientId].m_ClientsDummy = i;
					m_aClients[i].m_ClientsDummy = ClientId;
					break;
				}
			}
		}
