...
void CServer::ConStatus(IConsole::IResult *pResult, void *pUser)
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		{
			const char *pDnsbl = pThis->m_aClients[i].m_DnsblState == CClient::DNSBL_STATE_PENDING ? "pending" :
						 			pThis->m_aClients[i].m_DnsblState == CClient::DNSBL_STATE_BLACKLISTED ? "blacklisted" :
									pThis->m_aClients[i].m_DnsblState == CClient::DNSBL_STATE_WHITELISTED ? "whitelisted" : "";
			...
			if(CanSeeAddress)
				str_format(aBuf, sizeof(aBuf), "id=%d addr='%s' name='%s' client=%d secure=%s dnsbl='%s' %s", i, aAddrStr,
					pThis->m_aClients[i].m_aName, pThis->GameServer()->GetClientVersion(i), pThis->m_NetServer.HasSecurityToken(i) ? "yes" : "no", pDnsbl, aAuthStr);
			else
				str_format(aBuf, sizeof(aBuf), "id=%d name='%s' client=%d secure=%s dnsbl='%s' %s", i,
					pThis->m_aClients[i].m_aName, pThis->GameServer()->GetClientVersion(i), pThis->m_NetServer.HasSecurityToken(i) ? "yes" : "no", pDnsbl, aAuthStr);
		}
		else
		{
