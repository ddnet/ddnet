...
public:
	...
	int Players(const CBrowserEntry &Item) const
	{
		return g_Config.m_BrFilterSpectators ? Item.m_NumPlayers : Item.m_NumClients;
	}

	int Max(const CBrowserEntry &Item) const
	{
	...
	const CBrowserEntry *SortedGet(int Index) const;
	...
	void Set(const NETADDR &Addr, int Type, int Token, const CBrowserEntry *pInfo);
...
private:
	...
	void SetInfo(CServerEntry *pEntry, const CBrowserEntry &Info);
