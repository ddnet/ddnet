...
void CClient::DisconnectWithReason(const char *pReason)
{
	...
	{
		DemoRecorder_Stop(Recorder, Recorder == RECORDER_REPLAYS);
	}
...
void CClient::DemoRecorder_UpdateReplayRecorder()
{
	...
	if(g_Config.m_ClReplays && !DemoRecorder(RECORDER_REPLAYS)->IsRecording())
	{
		// Ensure that replay filenames are unique by appending a random number.
		// The replay demo is sliced in a background job, so we need to avoid
		// recording to the same file when restarting the replay recorder.
		char aFilename[IO_MAX_PATH_LENGTH];
		str_format(aFilename, sizeof(aFilename), "replays/replay_tmp-%s-%d", m_aCurrentMap, secure_rand());
...
{
	m_aDemoRecorder[Recorder].Stop(RemoveFile);
}
