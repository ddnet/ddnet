...
bool CSound::DecodeOpus(CSample &Sample, const void *pData, unsigned DataSize, const char *pContextName) const
{
	...
	if(pOpusFile)
	{
		...
		{
			op_free(pOpusFile);
			log_error("sound/opus", "File is not mono or stereo. Context='%s'", pContextName);
			return false;
		}
		...
		{
			op_free(pOpusFile);
			log_error("sound/opus", "Failed to get number of samples, error %d. Context='%s'", NumSamples, pContextName);
			return false;
		}
		...
		while(Pos < NumSamples)
		{
			...
			if(Read < 0)
			{
				free(pSampleData);
				op_free(pOpusFile);
				log_error("sound/opus", "op_read error %d at %d. Context='%s'", Read, Pos, pContextName);
				return false;
			}
			else if(Read == 0) // EOF
				break;
	...
	{
		log_error("sound/opus", "Failed to decode sample, error %d. Context='%s'", OpusError, pContextName);
		return false;
	}
...
bool CSound::DecodeWV(CSample &Sample, const void *pData, unsigned DataSize, const char *pContextName) const
{
	...
	if(pContext)
	{
		...
		if(NumChannels > 2)
		{
			log_error("sound/wv", "File is not mono or stereo. Context='%s'", pContextName);
			s_pWVBuffer = nullptr;
			return false;
		}
		...
		if(BitsPerSample != 16)
		{
			log_error("sound/wv", "Bits per sample is %d, not 16. Context='%s'", BitsPerSample, pContextName);
			s_pWVBuffer = nullptr;
			return false;
		}
		...
		{
			free(pBuffer);
			log_error("sound/wv", "WavpackUnpackSamples failed. NumSamples=%d NumChannels=%d Context='%s'", NumSamples, NumChannels, pContextName);
			s_pWVBuffer = nullptr;
			return false;
		}
	...
	{
		log_error("sound/wv", "Failed to decode sample (%s). Context='%s'", aError, pContextName);
		s_pWVBuffer = nullptr;
		return false;
	}
...
int CSound::LoadOpus(const char *pFilename, int StorageType)
{
	...
	if(!pSample)
	{
		log_error("sound/opus", "Failed to allocate sample ID. Context='%s'", pFilename);
		return -1;
	}
	...
	if(!m_pStorage->ReadFile(pFilename, StorageType, &pData, &DataSize))
	{
		...
		log_error("sound/opus", "Failed to open file. Context='%s'", pFilename);
		return -1;
	}
...
int CSound::LoadWV(const char *pFilename, int StorageType)
{
	...
	if(!pSample)
	{
		log_error("sound/wv", "Failed to allocate sample ID. Context='%s'", pFilename);
		return -1;
	}
	...
	{
		UnloadSample(pSample->m_Index);
		log_error("sound/wv", "Failed to open file. Context='%s'", pFilename);
		return -1;
	}
