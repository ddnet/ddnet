...
bool CSound::DecodeOpus(CSample &Sample, const void *pData, unsigned DataSize, const char *pFilename) const
{
	...
	if(pOpusFile)
	{
		...
		{
			op_free(pOpusFile);
			log_error("sound/opus", "File is not mono or stereo. Filename='%s'", pFilename);
			return false;
		}
		...
		{
			op_free(pOpusFile);
			log_error("sound/opus", "Failed to get number of samples, error %d. Filename='%s'", NumSamples, pFilename);
			return false;
		}
		...
		while(Pos < NumSamples)
		{
			...
			if(Read < 0)
			{
				free(pSampleData);
				op_free(pOpusFile);
				log_error("sound/opus", "op_read error %d at %d. Filename='%s'", Read, Pos, pFilename);
				return false;
			}
			else if(Read == 0) // EOF
				break;
	...
	{
		log_error("sound/opus", "Failed to decode sample, error %d. Filename='%s'", OpusError, pFilename);
		return false;
	}
...
bool CSound::DecodeWV(CSample &Sample, const void *pData, unsigned DataSize, const char *pFilename) const
{
	...
	if(pContext)
	{
		...
		if(NumChannels > 2)
		{
			log_error("sound/wv", "File is not mono or stereo. Filename='%s'", pFilename);
			s_pWVBuffer = nullptr;
			return false;
		}
		...
		if(BitsPerSample != 16)
		{
			log_error("sound/wv", "Bits per sample is %d, not 16. Filename='%s'", BitsPerSample, pFilename);
			s_pWVBuffer = nullptr;
			return false;
		}
		...
		{
			free(pBuffer);
			log_error("sound/wv", "WavpackUnpackSamples failed. NumSamples=%d NumChannels=%d Filename='%s'", NumSamples, NumChannels, pFilename);
			s_pWVBuffer = nullptr;
			return false;
		}
	...
	{
		log_error("sound/wv", "Failed to decode sample (%s). Filename='%s'", aError, pFilename);
		s_pWVBuffer = nullptr;
		return false;
	}
