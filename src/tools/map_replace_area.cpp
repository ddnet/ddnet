...
int main(int argc, const char *argv[])
{
	...
	dbg_msg("map_replace_area", "from_map='%s'; to_map='%s'; from_area='%fx,%fy'; to_area='%fx,%fy'; area_width='%fpx'; area_heigth='%fpx'; output_map='%s'",
		...
		pGameAreas[0][0][1] - pGameAreas[0][0][2], pGameAreas[0][1][1] - pGameAreas[0][1][0], pMapNames[2]);
...
void ReplaceAreaQuads(CDataFileReader InputMaps[2], const float pGameAreas[][2][2], const CMapItemGroup *pLayerGroups[2], CMapItemLayer *pItem[2], const int ItemNumber)
{
	...
	pQuads[3] = new CQuad[pQuadLayer[0]->m_NumQuads + pQuadLayer[1]->m_NumQuads];
	int QuadsCounter = 0;

	bool bDataChanged = RemoveDestinationQuads(pGameAreas[1], pQuads[1], pQuadLayer[1]->m_NumQuads, pLayerGroups[1], pQuads[3], QuadsCounter);
	bDataChanged |= InsertDestinationQuads(pGameAreas, pQuads[0], pQuadLayer[0]->m_NumQuads, pLayerGroups, pQuads[3], QuadsCounter);
...
MapObject CreateMapObject(const CMapItemGroup *pLayerGroup, const int PosX, const int PosY, const int Width, const int Height)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if(Ob.m_pSpeed[i] < 0)
			swap(Ob.m_pScreenOffset[i][0], Ob.m_pScreenOffset[i][1]);
	}
...
void SetExtendedArea(MapObject &Ob)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if(Ob.m_pExtendedArea[i][0] > Ob.m_pExtendedArea[i][1])
			swap(Ob.m_pExtendedArea[i][0], Ob.m_pExtendedArea[i][1]);
	}
}
