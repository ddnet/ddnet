...
{
	// A tee that took a speedup tile at rest carries a NaN velocity, so the conversions
	// below have to be the checked ones.
	pObjCore->m_X = round_to_int_checked(m_Pos.x);
	pObjCore->m_Y = round_to_int_checked(m_Pos.y);

	pObjCore->m_VelX = round_to_int_checked(m_Vel.x * 256.0f);
	pObjCore->m_VelY = round_to_int_checked(m_Vel.y * 256.0f);
	pObjCore->m_HookState = m_HookState;
	pObjCore->m_HookTick = m_HookTick;
	pObjCore->m_HookX = round_to_int_checked(m_HookPos.x);
	pObjCore->m_HookY = round_to_int_checked(m_HookPos.y);
	pObjCore->m_HookDx = round_to_int_checked(m_HookDir.x * 256.0f);
	pObjCore->m_HookDy = round_to_int_checked(m_HookDir.y * 256.0f);
