...
private:
	...
	void RenderTilemapImpl(T *pTiles, int w, int h, float Scale, ColorRGBA Color, const FFetchCallback &FetchIndexAndFlags, int RenderFlags, CTuneColorMapper *pTuneColorMapper = nullptr);

	template<typename T, typename FRenderTile>
	void RenderOverlay(T *pTiles, int w, int h, float Scale, const FRenderTile &RenderTile, float Alpha);
};
