...
void CMapRenderer::Load(ERenderType Type, CLayers *pLayers, IMapImages *pMapImages, IEnvelopeEval *pEnvelopeEval, std::optional<FRenderUploadCallback> RenderCallbackOptional)
{
	...
	std::shared_ptr<CEnvelopeHandler> pEnvelopeHandler = std::make_shared<CEnvelopeHandler>(pEnvelopeEval, pLayers->Map());
	...
	for(int GroupId = 0; GroupId < pLayers->NumGroups(); GroupId++)
	{
		...
		pRenderLayerGroup->OnInit(Graphics(), TextRender(), RenderMap(), pEnvelopeHandler, pLayers->Map(), pMapImages, RenderCallbackOptional);
		...
		for(int LayerId = 0; LayerId < pGroup->m_NumLayers; LayerId++)
		{
			...
			if(pRenderLayer)
			{
				pRenderLayer->OnInit(Graphics(), TextRender(), RenderMap(), pEnvelopeHandler, pLayers->Map(), pMapImages, RenderCallbackOptional);
