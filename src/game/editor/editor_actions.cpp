...
CEditorActionEnvelopeDelete::CEditorActionEnvelopeDelete(CEditor *pEditor, int EnvelopeIndex, std::vector<CEditorObjectReference> &ObjectReferences, std::shared_ptr<CEnvelope> &Envelope) :
	IEditorAction(pEditor), m_EnvelopeIndex(EnvelopeIndex), m_pEnv(Envelope), m_ObjectReferences(ObjectReferences)
{
...
void CEditorActionEnvelopeDelete::Undo()
{
	...
	m_pEditor->m_Map.InsertEnvelope(m_EnvelopeIndex, m_pEnv, m_ObjectReferences);
}
