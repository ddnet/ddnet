...
void CLayerGroup::Render()
{
	...
	if(m_UseClipping)
	{
		...
		int ClipX = (int)std::floor(Left * pGraphics->ScreenWidth() / ScreenWidth);
		int ClipY = (int)std::floor(Top * pGraphics->ScreenHeight() / ScreenHeight);

		pGraphics->ClipEnable(
			ClipX,
			ClipY,
			(int)std::ceil(Right * pGraphics->ScreenWidth() / ScreenWidth) - ClipX,
			(int)std::ceil(Bottom * pGraphics->ScreenHeight() / ScreenHeight) - ClipY);
	}
