...
public:
	...
	void InsertEnvelope(int Index, std::shared_ptr<CEnvelope> &Envelope, std::vector<CEditorObjectReference> &EditorObjectReferences);
	std::vector<CEditorObjectReference> DeleteEnvelope(int Index);
	int MoveEnvelope(int IndexFrom, int IndexTo);
	template<typename F>
	std::vector<CEditorObjectReference> VisitEnvelopeReferences(F &&Visitor);
