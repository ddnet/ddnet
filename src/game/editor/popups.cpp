...
CUi::EPopupMenuFunctionResult CEditor::PopupMenuSettings(void *pContext, CUIRect View, bool Active)
{
	...
	View.HSplitTop(12.0f, &Slot, &View);
	{
		...
		pEditor->Ui()->DoLabel(&Label, "Show in-game entities", 10.0f, TEXTALIGN_ML);

		static int s_ButtonNo = 0;
		static int s_ButtonYes = 0;
		if(pEditor->DoButton_Ex(&s_ButtonNo, "No", !g_Config.m_EdShowInGameEntities, &No, 0, "Do not show how weapons, shields, hearts and flags appear in-game", IGraphics::CORNER_L))
		{
			g_Config.m_EdShowInGameEntities = false;
		}
		if(pEditor->DoButton_Ex(&s_ButtonYes, "Yes", g_Config.m_EdShowInGameEntities, &Yes, 0, "Show how weapons, shields, hearts and flags appear in-game", IGraphics::CORNER_R))
		{
