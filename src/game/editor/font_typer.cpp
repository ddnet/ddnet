...
CFontTyper::CFontTyper()
{
	m_EditorMode = FT_MODE_NONE;
	m_TextIndexX = -1;
	m_TextIndexY = -1;
	m_TextLineLen = 0;
	m_NextCursorBlink = 0;
...
void CFontTyper::LoadMapresMetaFile(const char *pImage)
{
	char aFilepath[1024];
	...
	dbg_msg("editor", "loading mapres meta data for %s", pImage);
	if((*pJsonData)["type"].type == json_string)
		dbg_msg("editor", "  type=%s", (const char *)(*pJsonData)["type"]);
	if((*pJsonData)["offset_alphabet"].type == json_integer)
	{
		dbg_msg("editor", "  alphabet=%ld", (long int)(*pJsonData)["offset_alphabet"].u.integer);
		m_LetterOffset = (*pJsonData)["offset_alphabet"].u.integer - 1;
	}
	if((*pJsonData)["offset_numbers"].type == json_integer)
	{
		dbg_msg("editor", "  numbers=%ld", (long int)(*pJsonData)["offset_numbers"].u.integer);
...
void CFontTyper::Init(class CEditor *pEditor)
{
	m_pEditor = pEditor;
...
void CFontTyper::DoMapEditor()
{
	if(m_pEditor->UI()->MouseButton(0))
		SetCursor();
	if(m_pEditor->Input()->KeyPress(KEY_T) && m_EditorMode != FT_MODE_TEXT && m_pEditor->m_Dialog == DIALOG_NONE && m_pEditor->m_EditBoxActive == 0)
	{
		m_EditorMode = FT_MODE_TEXT;
