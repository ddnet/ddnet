...
void CEditor::RenderMenubar(CUIRect MenuBar)
{
	...
	{
		Ui()->DoPopupMenu(&m_EditorControlsPopupMenuId, Controls.x, Controls.y + Controls.h - 1.0f, 700.0f, 550.0f, this, PopupMenuControls, PopupProperties);
	}
...
void CEditor::Init()
{
	...
	RegisterBind(false, false, true, KEY_UNKNOWN, "Hold to axis lock the mouse (brush needs to be not empty).", EBindSection::BRUSH);
	...
	RegisterBind(false, false, true, KEY_UNKNOWN, "Ignore the grid.", EBindSection::QUADS_AND_SOUNDS);
	RegisterBind(true, false, false, KEY_UNKNOWN, "Move quad/sound points more precise.", EBindSection::QUADS_AND_SOUNDS);
	...
	RegisterBind(false, true, false, KEY_UNKNOWN, "Enter quad rotation mode.", EBindSection::QUADS_AND_SOUNDS);
...
void CEditor::OnRender()
{
	...
	if(m_Dialog == DIALOG_NONE &&
		CLineInput::GetActiveInput() == nullptr &&
		// Don't toggle the gui while the controls popup is open, as Tab is used to cycle through the search matches there.
		!Ui()->IsPopupOpen(&m_EditorControlsPopupMenuId) &&
		!Input()->ModifierIsPressed() &&
...
std::shared_ptr<CEditorBind> CEditor::RegisterBind(bool ShiftPressed, bool ModifierPressed, bool AltPressed, int Key, const char *pDescription, EBindSection Section)
{
	...
	dbg_assert(Key >= KEY_FIRST && Key < KEY_LAST, "Keybind invalid:%d, '%s'", Key, pDescription);
