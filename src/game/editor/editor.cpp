			...
			if(Index == ENTITY_DOOR)
			{
				for(size_t i = 0; i < sizeof(aOffsets) / sizeof(ivec2); ++i)
				{
					...
					if((IndexDoorLength >= ENTITY_LASER_SHORT && IndexDoorLength <= ENTITY_LASER_LONG) || IndexDoorLength == ENTITY_LASER_LEN)
					{
						...
						//dbg_msg("editor", "NumberDoorLength %d", NumberDoorLength);
						float Length = IndexDoorLength == ENTITY_LASER_LEN ? clamp(NumberDoorLength / 4.f, 0.5f, 64.f) : (IndexDoorLength - ENTITY_LASER_SHORT + 1) * 3;
