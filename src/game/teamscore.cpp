...
{
	return m_aTeam[ClientId1] == NUM_JOINABLE_TEAMS || m_aTeam[ClientId2] == NUM_JOINABLE_TEAMS || m_aTeam[ClientId1] == m_aTeam[ClientId2];
}
...
{
	dbg_assert(Team >= TEAM_FLOCK && Team < NUM_ALL_TEAMS, "invalid team %d", Team);
	m_aTeam[ClientId] = Team;
}

bool CTeamsCore::CanKeepHook(int ClientId1, int ClientId2) const
{
	if(m_aTeam[ClientId1] == (m_IsDDRace16 ? VANILLA_TEAM_SUPER : NUM_JOINABLE_TEAMS) || m_aTeam[ClientId2] == (m_IsDDRace16 ? VANILLA_TEAM_SUPER : NUM_JOINABLE_TEAMS) || ClientId1 == ClientId2)
		return true;
