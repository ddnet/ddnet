...
void CCharacter::HandleNinja()
{
	...
	if(m_Core.m_Ninja.m_CurrentMoveTime > 0)
	{
		...
		// check if we Hit anything along the way
		{
			...
			Num = GameWorld()->FindEntities(OldPos, Radius, apTargetEnts, MAX_TARGET_SWITCHES, CGameWorld::ENTTYPE_TARGETSWITCH);
...
void CCharacter::FireWeapon()
{
	...
	case WEAPON_HAMMER:
	{
		...
		Num = GameWorld()->FindEntities(ProjStartPos, GetProximityRadius() * 0.5f, apTargetEnts,
			MAX_TARGET_SWITCHES, CGameWorld::ENTTYPE_TARGETSWITCH);
