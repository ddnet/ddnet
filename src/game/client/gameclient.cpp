...
void CGameClient::OnConsoleInit()
{
	...
	Console()->Register("envelope_trigger", "i[zone] s[trigger_type] i[envelope]", CFGFLAG_GAME, ConEnvTrigger, this, "Set a trigger type for an env in a trigger zone");
	Console()->Register("tune_zone_envelope_trigger", "i[zone] i[envelope_zone]", CFGFLAG_GAME, ConTuneZoneEnvTrigger, this, "Make a tune zone activate an envelope zone");
	...
	// go through all the items in the snapshot and gather the info we want
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			else if(Item.m_Type == NETOBJTYPE_ENVELOPETRIGGER)
			{
				...
				if(EnvelopeId >= 0 &&
					pEnvelopeData->m_Type >= 0 && pEnvelopeData->m_Type < NUM_ENVELOPE_TRIGGERS &&
					pEnvelopeData->m_StartTick <= Client()->GameTick(g_Config.m_ClDummy) &&
					(ClientId == -1 || ClientId == m_Snap.m_LocalClientId))
				{
					...
					EEnvelopeTriggerType Type = static_cast<EEnvelopeTriggerType>(pEnvelopeData->m_Type);
					CEnvelopeTriggerState State(Type, pOldState);
...
void CGameClient::ConEnvTrigger(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(TriggerZoneId >= 0 && TriggerZoneId < 256 * 256)
	{
		...
		EnvTrigger.m_EnvelopeId = EnvelopeId;
		EnvTrigger.m_State = CEnvelopeTrigger::FromName(pTriggerName);

		TriggerZone.m_vEnvelopeTriggers.emplace_back(EnvTrigger);
	}
}
