...
void CGameClient::OnNewSnapshot()
{
	...
	if(Recordings != m_NumRecordings)
	{
		m_NumRecordings = Recordings;
		if(m_NumRecordings != 0)
		{
...
void CGameClient::UpdateLocalTuning()
{
	...
	if(!m_Snap.m_pLocalCharacter)
	{
		...
		{
			LocalPos = vec2(m_Snap.m_pSpectatorInfo->m_X, m_Snap.m_pSpectatorInfo->m_Y);
		}
		else
		{
			return;
	...
	{
		LocalPos = vec2(m_Snap.m_pLocalCharacter->m_X, m_Snap.m_pLocalCharacter->m_Y);
	}
	...
	if(m_GameWorld.m_WorldConfig.m_UseTuneZones)
	{
		...
		if(m_NumRecordings != 0 && m_IsDummySwapping && m_aLocalTuneZone[0] != m_aLocalTuneZone[1])
		{
			CMsgPacker Msg(NETMSGTYPE_SV_TUNEPARAMS);
			int *pParams = (int *)&m_aTuning[g_Config.m_ClDummy];
			for(unsigned i = 0; i < sizeof(m_aTuning[0]) / sizeof(int); i++)
