...
void CGameClient::UpdateSpectatorCursor()
{
	...
	if(!g_Config.m_Debug)
	{
		...
		m_CursorInfo.m_Target = mix_polynomial(aTime, aData, CCursorInfo::SAMPLE_FRAME_WINDOW, DisplayTime);
	}
	else
	{
		m_CursorInfo.m_Target = Target;
		m_CursorInfo.m_NumSamples = 0;
	}
	...
	const float Zoom = (m_Camera.SpectatingCameraActivated() && m_Camera.m_AutoSpecCamera && m_Camera.CanUseAutoSpecCamera()) ? m_Camera.m_Zoom : m_Snap.m_SpecInfo.m_Zoom;
...
{
	m_Camera.SetZoom(std::pow(CCamera::ZOOM_STEP, g_Config.m_ClDefaultZoom - 10), g_Config.m_ClSmoothZoomTime, true);
	m_MultiViewPersonalZoom = 0.0f;
	m_MultiViewActivated = false;
