	...
	float Intra)
{
	...
	if(g_Config.m_ClSparkleEffect && Local && !(RenderInfo.m_TeeRenderFlags & TEE_EFFECT_FROZEN))
		GameClient()->m_Effects.PlayerSparkle(BodyPos, Alpha);
	if(g_Config.m_ClSparkleEffectOthers && !Local && !(RenderInfo.m_TeeRenderFlags & TEE_EFFECT_FROZEN))
		GameClient()->m_Effects.PlayerSparkle(BodyPos, Alpha);

	
	if (g_Config.m_ClPlayerTrail && Local)
	{
		
		ColorRGBA rgb = ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f);


		Graphics()->DrawRect(BodyPos.x - 25, BodyPos.y - 25, 50, 50, rgb, IGraphics::CORNER_ALL, 0.0f);
...
void CPlayers::OnRender()
{
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		if(g_Config.m_ClOwnTeeSkin && m_pClient->m_Snap.m_LocalClientId)
		{
			// change the skin for the player to the ninja
			const auto *pSkin = m_pClient->m_Skins.FindOrNullptr(g_Config.m_ClOwnTeeSkinName);
			if(pSkin != nullptr && !Frozen)
			{
				aRenderInfo[m_pClient->m_Snap.m_LocalClientId].m_aSixup[g_Config.m_ClDummy].Reset();

				aRenderInfo[m_pClient->m_Snap.m_LocalClientId].Apply(pSkin);
				aRenderInfo[m_pClient->m_Snap.m_LocalClientId].m_CustomColoredSkin = IsTeamplay;
				if(g_Config.m_ClOwnTeeSkinCustomColor)
				{
					aRenderInfo[m_pClient->m_Snap.m_LocalClientId].m_CustomColoredSkin = true;
				}
				else
				{
					aRenderInfo[m_pClient->m_Snap.m_LocalClientId].m_ColorBody = ColorRGBA(1, 1, 1);
					aRenderInfo[m_pClient->m_Snap.m_LocalClientId].m_ColorFeet = ColorRGBA(1, 1, 1);
				}
			}
		}
