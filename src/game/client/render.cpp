		...
		for(int f = 0; f < 2; f++)
		{
			...
			if(f == 1)
			{
				...
				if(pInfo->m_aTextures[protocol7::SKINPART_DECORATION].IsValid())
				{
					...
					Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_DECORATION]);
				...
				{
					Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_BODY]);
				...
				if(pInfo->m_aTextures[protocol7::SKINPART_MARKING].IsValid() && !OutLine)
				{
					...
					Graphics()->QuadsBegin();
					Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle * pi * 2);
					ColorRGBA MarkingColor = pInfo->m_aColors[protocol7::SKINPART_MARKING];
				...
				if(IsBot)
				{
					...
					Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_EYES]);
				if(p == 1)
				{
					switch(Emote)
					{
