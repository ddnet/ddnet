...
CScoreboard::CScoreboard() { OnReset(); }
...
void CScoreboard::OnRelease() { m_Active = false; }
	...
	if (m_pClient->m_Snap.m_pGameInfoObj)
	{
		if (m_pClient->m_Snap.m_pGameInfoObj->m_ScoreLimit)
		{
			...
			str_format(aBuf, sizeof(aBuf), "%s: %d", Localize("Score limit"),
					   m_pClient->m_Snap.m_pGameInfoObj->m_ScoreLimit);
		...
		if (m_pClient->m_Snap.m_pGameInfoObj->m_TimeLimit)
		{
			...
			str_format(aBuf, sizeof(aBuf), Localize("Time limit: %d min"),
					   m_pClient->m_Snap.m_pGameInfoObj->m_TimeLimit);
		...
		{
			char aBuf[64];
			str_format(aBuf, sizeof(aBuf), "%s %d/%d", Localize("Round"),
					   m_pClient->m_Snap.m_pGameInfoObj->m_RoundCurrent, m_pClient->m_Snap.m_pGameInfoObj->m_RoundNum);
...
void CScoreboard::RenderSpectators(float x, float y, float w)
{
	...
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		if (!pInfo || pInfo->m_Team != TEAM_SPECTATORS) continue;

		if (Multiple) TextRender()->TextEx(&Cursor, ", ", 2);
...
{
	if (Team == TEAM_SPECTATORS) return;
	...
	float CountryOffset = PingOffset - (LineHeight - Spacing - TeeSizeMod * 5.0f) * 2.0f,
		  CountryLength = (LineHeight - Spacing - TeeSizeMod * 5.0f) * 2.0f;
	...
	const char *pScore =
		(m_IsGameTypeRace && g_Config.m_ClDDRaceScoreBoard && m_pClient->m_AllowTimeScore[g_Config.m_ClDummy])
			? Localize("Time")
			: Localize("Score");
	float ScoreWidth = TextRender()->TextWidth(0, HeadlineFontsize, pScore, -1);
	tw = ScoreLength > ScoreWidth ? ScoreLength : ScoreWidth;
	TextRender()->Text(0, ScoreOffset + ScoreLength - tw, y + (HeadlineFontsize * 2.f - HeadlineFontsize) / 2.f,
					   HeadlineFontsize, pScore, -1);

	TextRender()->Text(0, NameOffset, y + (HeadlineFontsize * 2.f - HeadlineFontsize) / 2.f, HeadlineFontsize,
					   Localize("Name"), -1);

	tw = TextRender()->TextWidth(0, HeadlineFontsize, Localize("Clan"), -1);
	TextRender()->Text(0, ClanOffset + ClanLength / 2 - tw / 2, y + (HeadlineFontsize * 2.f - HeadlineFontsize) / 2.f,
					   HeadlineFontsize, Localize("Clan"), -1);

	tw = TextRender()->TextWidth(0, HeadlineFontsize, Localize("Ping"), -1);
	TextRender()->Text(0, PingOffset + PingLength - tw, y + (HeadlineFontsize * 2.f - HeadlineFontsize) / 2.f,
					   HeadlineFontsize, Localize("Ping"), -1);
	...
	int rendered = 0;
	if (upper16) rendered = -16;
	if (upper32) rendered = -32;
	if (upper24) rendered = -24;
	...
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (!pInfo || pInfo->m_Team != Team) continue;
		...
		if (DDTeam != TEAM_FLOCK)
		{
			...
			if (OldDDTeam != DDTeam) Corners |= CUI::CORNER_TL | CUI::CORNER_TR;
			if (NextDDTeam != DDTeam) Corners |= CUI::CORNER_BL | CUI::CORNER_BR;
			...
			if (NextDDTeam != DDTeam)
			{
				...
				if (m_pClient->m_Snap.m_aTeamSize[0] > 8)
				{
					...
					TextRender()->SetCursor(&Cursor, x - 10.0f, y + Spacing + FontSize - (FontSize / 1.5f),
											FontSize / 1.5f, TEXTFLAG_RENDER | TEXTFLAG_STOP_AT_END);
				...
				else
				{
					...
					TextRender()->SetCursor(&Cursor, ScoreOffset + w / 2.0f - tw / 2.0f, y + LineHeight,
											FontSize / 1.5f, TEXTFLAG_RENDER | TEXTFLAG_STOP_AT_END);
					Cursor.m_LineWidth = NameLength + 3;
				}
		...
		tw = TextRender()->TextWidth(0, FontSize, aBuf, -1);
		TextRender()->SetCursor(&Cursor, ScoreOffset + ScoreLength - tw, y + (LineHeight - FontSize) / 2.f, FontSize,
								TEXTFLAG_RENDER);
		TextRender()->TextEx(&Cursor, aBuf, -1);
			 ...
			 m_pClient->m_Snap.m_pGameDataObj->m_FlagCarrierBlue == pInfo->m_ClientID))
		{
			...
			RenderTools()->SelectSprite(pInfo->m_Team == TEAM_RED ? SPRITE_FLAG_BLUE : SPRITE_FLAG_RED,
										SPRITE_FLAG_FLIP_X);
		...
		RenderTools()->RenderTee(CAnimState::GetIdle(), &TeeInfo, EMOTE_NORMAL, vec2(1.0f, 0.0f),
								 vec2(TeeOffset + TeeLength / 2, y + LineHeight / 2));

		// name
		TextRender()->SetCursor(&Cursor, NameOffset, y + (LineHeight - FontSize) / 2.f, FontSize,
		...
		m_pClient->m_pCountryFlags->Render(m_pClient->m_aClients[pInfo->m_ClientID].m_Country, &Color, CountryOffset,
										   y + (Spacing + TeeSizeMod * 5.0f) / 2.0f, CountryLength,
...
void CScoreboard::OnRender()
{
	...
	if (m_pClient->m_pMotd->IsActive()) m_pClient->m_pMotd->Clear();
	...
	if (m_pClient->m_Snap.m_pGameInfoObj)
	{
		...
		else
		{
				...
				m_pClient->m_Snap.m_pGameDataObj)
			{
				...
				if (m_pClient->m_Snap.m_pGameDataObj->m_TeamscoreRed >
					m_pClient->m_Snap.m_pGameDataObj->m_TeamscoreBlue)
				{
					if (pRedClanName)
				...
				else if (m_pClient->m_Snap.m_pGameDataObj->m_TeamscoreBlue >
			...
			RenderScoreboard(Width / 2 - w - 5.0f, 150.0f, w, TEAM_RED,
							 pRedClanName ? pRedClanName : Localize("Red team"));
			RenderScoreboard(Width / 2 + 5.0f, 150.0f, w, TEAM_BLUE,
							 pBlueClanName ? pBlueClanName : Localize("Blue team"));
		}
	}
...
bool CScoreboard::Active()
{
	...
	if (m_pClient->m_pStatboard->IsActive()) return false;
	...
	if (m_pClient->m_Snap.m_pLocalInfo && m_pClient->m_Snap.m_pLocalInfo->m_Team != TEAM_SPECTATORS)
	{
		...
		if (!m_pClient->m_Snap.m_pLocalCharacter && g_Config.m_ClScoreboardOnDeath) return true;
	}
