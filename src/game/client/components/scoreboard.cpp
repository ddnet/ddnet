...
void CScoreboard::RenderTitle(CUIRect TitleBar, int Team, const char *pTitle)
{
	...
	const float ScoreTextWidthCentiseconds = TextRender()->TextWidth(TitleFontSize, aCentiSeconds);
	const float ScoreTextWidth = TextRender()->TextWidth(TitleFontSize, aScore) + ScoreTextWidthCentiseconds;
	...
	if(aScore[0] != '\0')
	{
		if(ScoreTextWidthCentiseconds > 0.0f)
		{
			...
			CentisecondLabel.HSplitTop(CentisecondLabel.h * 0.85f, &CentisecondLabel, nullptr);
			Ui()->DoLabel(&ScoreLabel, aScore, TitleFontSize, TEXTALIGN_MR);
			Ui()->DoLabel(&CentisecondLabel, aCentiSeconds, TitleFontSize * 0.7f, TEXTALIGN_ML);
		}
		else
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			else if(TimeScore)
			{
				...
				if(GameClient()->m_GameInfo.m_TimeType == ETimeType::TIME_TYPE_MILLISECONDS && pInfo->m_Score != -9999 && pInfo->m_Score < 1000 * 60 * 60)
				{
					...
					CentisecondOffset = TextRender()->TextWidth(FontSize * 0.7f, aBuf);
					TextRender()->Text(ScoreOffset + ScoreLength - CentisecondOffset, Row.y + (Row.h - FontSize * 0.85f) / 2.0f, FontSize * 0.7f, aBuf);
				}
			...
			TextRender()->Text(ScoreOffset + ScoreLength - TextRender()->TextWidth(FontSize, aBuf) - CentisecondOffset, Row.y + (Row.h - FontSize) / 2.0f, FontSize, aBuf);
