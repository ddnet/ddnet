		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(m_MouseUnlocked)
			{
				...
				if(ButtonResult != 0)
				{
					...
					Ui()->DoPopupMenu(&m_ScoreboardPopupContext, Ui()->MouseX(), Ui()->MouseY(), 110.0f, 120.0f, &m_ScoreboardPopupContext, PopupScoreboard);
				}
...
void CScoreboard::OnRender()
{
	...
	if(!IsActive() || GameClient()->m_Menus.IsActive())
		return;
...
CUi::EPopupMenuFunctionResult CScoreboard::PopupScoreboard(void *pContext, CUIRect View, bool Active)
{
	...
	bool IsLocal = pScoreboard->GameClient()->m_aLocalIds[0] == pPopupContext->m_ClientId ||
		       (pScoreboard->Client()->DummyConnected() && pScoreboard->GameClient()->m_aLocalIds[1] == pPopupContext->m_ClientId);
