...
void CScoreboard::RenderPlayerPopUp()
{
	...
	{
		RenderTooltip(m_Tooltip.m_pText, m_Tooltip.m_Pos.x, m_Tooltip.m_Pos.y);
		m_Tooltip.Clear();
	}

}

void CScoreboard::RenderTooltip(const char *pText, float x, float y)
{
	float FontSize = 10.0f;
	float TextWidth = TextRender()->TextWidth(FontSize, pText, -1, -1.0f);
	CUIRect TooltipRect = {x, y, TextWidth + 10.0f, 18.0f};
	TooltipRect.Draw(ColorRGBA(0, 0, 0, 0.75f), IGraphics::CORNER_ALL, 4.0f);
...
void CScoreboard::RenderQuickActions(CUIRect *pBase)
{
	...
	if(Hovered(&Action))
	{
		m_Tooltip.m_pText = IsFriend ? "Remove Friend" : "Add Friend";
	...
	// spec
	bool IsSpec = GameClient()->m_Snap.m_SpecInfo.m_Active;
	if(Hovered(&Action))
	{
		m_Tooltip.m_pText = IsSpec ? "Stop Spectating" : "Start Spectating";
		m_Tooltip.m_Pos = MousePos - vec2(-10.0f, 10.0f);
		...
		DoIconButton(&Action, IsSpec ? FontIcons::FONT_ICON_EYE_SLASH : FontIcons::FONT_ICON_EYE, SPopupProperties::ms_IconFontSize, TextRender()->DefaultTextColor());
	}
	else
	{
	...
	if(DoButtonLogic(&Action))
	{
		...
		if(IsSpec)
		{
			GameClient()->m_Spectator.StopSpectating();
		}
		else
		{
