	...
	for(const CNetObj_PlayerInfo *pInfo : GameClient()->m_Snap.m_apInfoByName)
	{
		...
		if(m_MouseUnlocked)
		{
			...
			if(ButtonResult != 0)
			{
				...
				// Only request info when the player is clicked, to avoid spamming the server.
				// There may be a very slight delay when opening the menu before the info becomes available
				GameClient()->RequestPlayerDetails(pInfo->m_ClientId);
...
CUi::EPopupMenuFunctionResult CScoreboard::CScoreboardPopupContext::Render(void *pContext, CUIRect View, bool Active)
{
	...
	View.HSplitTop(ItemSpacing * 2, nullptr, &View);
	pUi->DoLabel(&View, pScoreboard->GameClient()->m_aaPlayerDetails[pPopupContext->m_ClientId], FontSize, TEXTALIGN_MC);
