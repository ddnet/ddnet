...
private:
	...
	std::optional<bool> ParseBoolean(const json_value *pBooleanValue, const char *pAttributeName) const;
	...
	bool m_HideAllButtons = false; // Only takes effect while not in editor and when m_ToggleVisibilityGesture == true.

	bool m_ZoomGestureGame = false;
	bool m_ZoomGestureSpec = false;
	bool m_ToggleVisibilityGesture = false;
	...
	std::optional<vec2> m_ZoomStartPosGame = vec2(0.0f, 0.0f);
...
public:
	...
	bool HasEnabledToggleVisibilityGesture() const { return m_ToggleVisibilityGesture; }
	void SetToggleVisibilityGesture(bool EnabledToggleVisibilityGesture);
