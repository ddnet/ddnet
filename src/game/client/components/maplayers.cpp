...
{
	MapRender::EnvelopeEval(TimeOffsetMillis, Env, Result, Channels, this->m_pLayers->Map(), this->m_pEnvelopePoints.get(), this->Client(), this->GameClient(), this->m_OnlineOnly);
}
...
void CMapLayers::OnMapLoad()
{
	...
	m_pMapData = std::make_shared<IMapDataComponent>(m_pLayers->Map(), GameClient());
	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		pRenderLayerGroup->OnInit(GameClient(), m_pMapData.get(), m_pImages, m_OnlineOnly);
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if(m_Type == MapRender::TYPE_BACKGROUND_FORCE || m_Type == MapRender::TYPE_BACKGROUND)
			{
				if(PassedGameLayer)
					return;
			}
			else if(m_Type == MapRender::TYPE_FOREGROUND)
			{
				if(!PassedGameLayer)
					continue;
			}
			...
			if(pRenderLayer)
			{
				pRenderLayer->OnInit(GameClient(), m_pMapData.get(), m_pImages, m_OnlineOnly);
...
void CMapLayers::OnRender()
{
	...
	Params.EntityOverlayVal = m_Type == MapRender::TYPE_FULL_DESIGN ? 0 : g_Config.m_ClOverlayEntities;
	Params.m_RenderType = m_Type;
	Params.m_Center = GetCurCamera()->m_Center;
	Params.m_Zoom = GetCurCamera()->m_Zoom;
	Params.m_DisableTileBuffer = false;
	...
	if(m_Type != MapRender::TYPE_BACKGROUND && m_Type != MapRender::TYPE_BACKGROUND_FORCE)
	{
