	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if(pLayer->m_Type == LAYERTYPE_TILES)
			{
				...
				{
					if(LayerType == LAYER_SWITCH)
						TextureId = MAP_IMAGE_ENTITY_LAYER_TYPE_SWITCH;
					else if(LayerType != LAYER_SPEEDUP)
						TextureId = MAP_IMAGE_ENTITY_LAYER_TYPE_ALL_EXCEPT_SWITCH;
...
void CMapLayers::RenderTilelayerWithTileBuffer(int TextureId, int LayerType, int TileLayerCounter, const ColorRGBA &Color)
{
	if(LayerType == LAYER_SPEEDUP)
		Graphics()->WrapClamp();

	if(TextureId != -1)
		Graphics()->TextureSet(m_pImages->Get(TextureId));
	else
		Graphics()->TextureClear();
...
void CMapLayers::RenderTilelayerNoTileBuffer(int TextureId, int LayerType, void *pTilesData, CMapItemLayerTilemap *pLayerTilemap, const ColorRGBA &Color)
{
	if(TextureId != -1)
		Graphics()->TextureSet(m_pImages->Get(TextureId));
	else
