	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if(pLayer->m_Type == LAYERTYPE_TILES && Graphics()->IsTileBufferingEnabled())
			{
				...
				int DataIndex;
				int TileLayerAndOverlayCount = GetTileLayerAndOverlayCount(pTMap, LayerType, &DataIndex);
...
void CMapLayers::OnRender()
{
	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if((Render && EntityOverlayVal < 100 && !IsEntityLayer) || (EntityOverlayVal && LayerType == LAYER_GAME) || (m_Type == TYPE_BACKGROUND_FORCE))
			{
				if(pLayer->m_Type == LAYERTYPE_TILES)
				{
					CMapItemLayerTilemap *pTMap = (CMapItemLayerTilemap *)pLayer;
...
int CMapLayers::GetTileLayerAndOverlayCount(const CMapItemLayerTilemap *pLayerTilemap, int LayerType, int *pDataIndex) const
{
	int DataIndex;
