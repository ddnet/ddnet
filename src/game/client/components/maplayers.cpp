...
void CMapLayers::EnvelopeEval(int TimeOffsetMillis, int Env, ColorRGBA &Result, size_t Channels, IMap *pMap, IClient *pClient, CGameClient *pGameClient)
{
	int EnvStart, EnvNum;
	...
	if(pClient && pGameClient && (pItem->m_Version < 2 || pItem->m_Synchronized))
	{
		if(pGameClient->m_Snap.m_pGameInfoObj)
		{
			// get the lerp of the current tick and prev
			const auto TickToNanoSeconds = std::chrono::nanoseconds(1) / (int64_t)pClient->GameTickSpeed();
...
void CMapLayers::EnvelopeEval(int TimeOffsetMillis, int Env, ColorRGBA &Result, size_t Channels, CMapLayers *pUser)
{
	EnvelopeEval(TimeOffsetMillis, Env, Result, Channels, pUser->m_pLayers->Map(), pUser->Client(), pUser->m_pClient);
}
...
void CMapLayers::OnMapLoad()
{
	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			pRenderLayer->OnInit(Graphics(), m_pLayers->Map(), RenderTools(), m_pImages, Client(), m_pClient);
