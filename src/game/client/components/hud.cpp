...
void CHud::OnInit()
{
	...
	{
		float ScaleX, ScaleY;
		Graphics()->GetSpriteScaleImpl(g_pData->m_Weapons.m_aId[i].m_pSpriteCursor->m_W, g_pData->m_Weapons.m_aId[i].m_pSpriteCursor->m_H, ScaleX, ScaleY);
...
void CHud::PreparePlayerStateQuads()
{
	...
	for(int Weapon = 0; Weapon < NUM_WEAPONS; ++Weapon)
	{
		...
		float ScaleX, ScaleY;
		Graphics()->GetSpriteScaleImpl(WeaponSpec.m_pSpriteBody->m_W, WeaponSpec.m_pSpriteBody->m_H, ScaleX, ScaleY);
