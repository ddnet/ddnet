...
void CHud::RenderSpectatorHud()
{
	...
	if(m_pClient->m_Camera.SpectatingCameraActivated() && m_pClient->m_Camera.CanUseAutoSpecCamera())
	{
		bool AutoSpecCameraEnabled = m_pClient->m_Camera.m_AutoSpecCamera;
		Graphics()->DrawRect(m_Width - 40.0f, m_Height - 12.0f, 32.0f, 10.0f, ColorRGBA(0.84f, 0.53f, 0.17f, AutoSpecCameraEnabled ? 0.85f : 0.25f), IGraphics::CORNER_ALL, 2.5f);
		TextRender()->TextColor(1, 1, 1, AutoSpecCameraEnabled ? 1.0f : 0.65f);
		TextRender()->SetFontPreset(EFontPreset::ICON_FONT);
		TextRender()->Text(m_Width - 40.0f + 3.0f, m_Height - 10.0f, 6.0f, FontIcons::FONT_ICON_CAMERA, -1.0f);
		TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);
		str_copy(aBuf, "AUTO");
		TextRender()->Text(m_Width - 20.0f - TextRender()->TextWidth(6.0f, aBuf) / 2.0f, m_Height - 10.0f, 6.0f, aBuf, -1.0f);
		TextRender()->TextColor(1, 1, 1, 1);
	}
}
