...
void CMenus::RenderDemoPlayer(CUIRect MainView)
{
	...
	if(m_DemoPlayerState == DEMOPLAYER_SLICE_SAVE)
	{
		...
		{
			if(str_comp(m_Demos[m_DemolistSelectedIndex].m_aFilename, m_aCurrentDemoFile) == 0)
	...
	{
		str_copy(m_aCurrentDemoFile, m_Demos[m_DemolistSelectedIndex].m_aFilename, sizeof(m_aCurrentDemoFile));
		m_aDemoPlayerPopupHint[0] = '\0';
...
void CMenus::DemolistPopulate()
{
	...
	std::stable_sort(m_Demos.begin(), m_Demos.end());
}
...
void CMenus::DemolistOnUpdate(bool Reset)
{
	...
	m_DemolistSelectedIndex = Reset ? !m_Demos.empty() ? 0 : -1 :
					  m_DemolistSelectedIndex >= (int)m_Demos.size() ? m_Demos.size() - 1 : m_DemolistSelectedIndex;
	m_DemolistSelectedIsDir = m_DemolistSelectedIndex < 0 ? false : m_Demos[m_DemolistSelectedIndex].m_IsDir;
}
...
void CMenus::RenderDemoList(CUIRect MainView)
{
	...
	{
		CDemoItem &Item = m_Demos[m_DemolistSelectedIndex];
	...
	if(!m_DemolistSelectedIsDir && m_DemolistSelectedIndex >= 0 && m_Demos[m_DemolistSelectedIndex].m_Valid)
	{
		...
		char aTimestamp[256];
		str_timestamp_ex(m_Demos[m_DemolistSelectedIndex].m_Date, aTimestamp, sizeof(aTimestamp), FORMAT_SPACE);
		...
		UI()->DoLabelScaled(&Right, m_Demos[m_DemolistSelectedIndex].m_Info.m_aType, 14.0f, TEXTALIGN_LEFT);
		Labels.HSplitTop(5.0f, 0, &Labels);
		Labels.HSplitTop(20.0f, &Left, &Labels);
		Left.VSplitLeft(150.0f, &Left, &Right);
		UI()->DoLabelScaled(&Left, Localize("Length:"), 14.0f, TEXTALIGN_LEFT);
		int Length = m_Demos[m_DemolistSelectedIndex].Length();
		char aBuf[64];
		...
		str_format(aBuf, sizeof(aBuf), "%d", m_Demos[m_DemolistSelectedIndex].m_Info.m_Version);
		UI()->DoLabelScaled(&Right, aBuf, 14.0f, TEXTALIGN_LEFT);
		Labels.HSplitTop(5.0f, 0, &Labels);
		Labels.HSplitTop(20.0f, &Left, &Labels);
		Left.VSplitLeft(150.0f, &Left, &Right);
		UI()->DoLabelScaled(&Left, Localize("Markers:"), 14.0f, TEXTALIGN_LEFT);
		str_format(aBuf, sizeof(aBuf), "%d", m_Demos[m_DemolistSelectedIndex].NumMarkers());
		UI()->DoLabelScaled(&Right, aBuf, 14.0f, TEXTALIGN_LEFT);
		...
		UI()->DoLabelScaled(&Right, m_Demos[m_DemolistSelectedIndex].m_Info.m_aMapName, 14.0f, TEXTALIGN_LEFT);
		Labels.HSplitTop(5.0f, 0, &Labels);
		Labels.HSplitTop(20.0f, &Left, &Labels);
		Left.VSplitLeft(150.0f, &Left, &Right);
		UI()->DoLabelScaled(&Left, Localize("Size:"), 14.0f, TEXTALIGN_LEFT);
		const float Size = m_Demos[m_DemolistSelectedIndex].Size() / 1024.0f;
		if(Size > 1024)
		...
		Labels.HSplitTop(20.0f, &Left, &Labels);
		Left.VSplitLeft(150.0f, &Left, &Right);
		if(m_Demos[m_DemolistSelectedIndex].m_MapInfo.m_Sha256 != SHA256_ZEROED)
		{
			UI()->DoLabelScaled(&Left, "SHA256:", 14.0f, TEXTALIGN_LEFT);
			char aSha[SHA256_MAXSTRSIZE];
			sha256_str(m_Demos[m_DemolistSelectedIndex].m_MapInfo.m_Sha256, aSha, sizeof(aSha) / 2);
		...
		else
		{
			...
			str_format(aBuf, sizeof(aBuf), "%08x", m_Demos[m_DemolistSelectedIndex].m_MapInfo.m_Crc);
			UI()->DoLabelScaled(&Right, aBuf, 14.0f, TEXTALIGN_LEFT);
		}
		Labels.HSplitTop(5.0f, 0, &Labels);
		Labels.HSplitTop(20.0f, &Left, &Labels);
		...
		UI()->DoLabelScaled(&Right, m_Demos[m_DemolistSelectedIndex].m_Info.m_aNetversion, 14.0f, TEXTALIGN_LEFT);
	}
	...
	*/
	...
	HandleListInputs(ListBox, s_ScrollValue, 3.0f, &m_ScrollOffset, s_aCols[0].m_Rect.h, m_DemolistSelectedIndex, m_Demos.size());
	if(PreviousIndex != m_DemolistSelectedIndex)
	{
		str_copy(g_Config.m_UiDemoSelected, m_Demos[m_DemolistSelectedIndex].m_aName, sizeof(g_Config.m_UiDemoSelected));
		DemolistOnUpdate(false);
	}
	...
	int ScrollNum = maximum<int>(m_Demos.size() - Num + 1, 0);
	...
	if(DoButton_Menu(&s_PlayButton, m_DemolistSelectedIsDir ? Localize("Open") : Localize("Play", "Demo browser"), 0, &PlayRect) || Activated || (Input()->KeyPress(KEY_P) && m_pClient->m_GameConsole.IsClosed() && m_DemoPlayerState == DEMOPLAYER_NONE))
	{
		if(m_DemolistSelectedIndex >= 0)
		{
			...
			{
				if(str_comp(m_Demos[m_DemolistSelectedIndex].m_aFilename, "..") == 0) // parent folder
					fs_parent_dir(m_aCurrentDemoFolder);
				else // sub folder
				{
					char aTemp[256];
					str_copy(aTemp, m_aCurrentDemoFolder, sizeof(aTemp));
					str_format(m_aCurrentDemoFolder, sizeof(m_aCurrentDemoFolder), "%s/%s", aTemp, m_Demos[m_DemolistSelectedIndex].m_aFilename);
					m_DemolistStorageType = m_Demos[m_DemolistSelectedIndex].m_StorageType;
				}
				DemolistPopulate();
				DemolistOnUpdate(true);
			}
			else // file
			{
				...
				const char *pError = Client()->DemoPlayer_Play(aBuf, m_Demos[m_DemolistSelectedIndex].m_StorageType);
				if(pError)
