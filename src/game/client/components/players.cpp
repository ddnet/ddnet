...
void CPlayers::OnInit()
{
	...
	{
		float ScaleX, ScaleY;
		Graphics()->GetSpriteScaleImpl(g_pData->m_Weapons.m_aId[i].m_pSpriteBody->m_W, g_pData->m_Weapons.m_aId[i].m_pSpriteBody->m_H, ScaleX, ScaleY);
		...
		for(int n = 0; n < g_pData->m_Weapons.m_aId[i].m_NumSpriteMuzzles; ++n)
		{
			if(g_pData->m_Weapons.m_aId[i].m_aSpriteMuzzles[n])
			{
				if(i == WEAPON_GUN || i == WEAPON_SHOTGUN)
				{
					...
					Graphics()->GetSpriteScaleImpl(g_pData->m_Weapons.m_aId[i].m_aSpriteMuzzles[n]->m_W, g_pData->m_Weapons.m_aId[i].m_aSpriteMuzzles[n]->m_H, ScaleX, ScaleY);
			}
