...
void CMenus::FriendlistOnUpdate()
{
	...
	for(int i = 0; i < m_pClient->Friends()->NumFriends(); ++i)
		m_Friends.emplace_back(m_pClient->Friends()->GetFriend(i));
...
void CMenus::RenderServerbrowserFriends(CUIRect View)
{
	...
	static float s_ScrollValue = 0;
	if(m_FriendlistSelectedIndex >= (int)m_Friends.size())
		m_FriendlistSelectedIndex = m_Friends.size() - 1;
	UiDoListboxStart(&m_Friends, &List, 30.0f, "", "", m_Friends.size(), 1, m_FriendlistSelectedIndex, s_ScrollValue);
	...
	if(Activated && !m_EnterPressed && m_Friends[m_FriendlistSelectedIndex].m_NumFound)
	{
		bool Found = false;
		int NumServers = ServerBrowser()->NumSortedServers();
		for(int i = 0; i < NumServers && !Found; i++)
		{
			...
			if(pItem->m_FriendState != IFriends::FRIEND_NO)
			{
				...
				{
					if(pItem->m_aClients[j].m_FriendState != IFriends::FRIEND_NO &&
						((g_Config.m_ClFriendsIgnoreClan && m_Friends[m_FriendlistSelectedIndex].m_pFriendInfo->m_aName[0]) || str_quickhash(pItem->m_aClients[j].m_aClan) == m_Friends[m_FriendlistSelectedIndex].m_pFriendInfo->m_ClanHash) &&
						(!m_Friends[m_FriendlistSelectedIndex].m_pFriendInfo->m_aName[0] ||
							str_quickhash(pItem->m_aClients[j].m_aName) == m_Friends[m_FriendlistSelectedIndex].m_pFriendInfo->m_NameHash))
					{
						str_copy(g_Config.m_UiServerAddress, pItem->m_aAddress, sizeof(g_Config.m_UiServerAddress));
