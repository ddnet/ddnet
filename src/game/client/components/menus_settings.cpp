...
void CMenus::RenderSettingsAppearance(CUIRect MainView)
{
	...
	else if(s_CurTab == APPEARANCE_TAB_HOOK_COLLISION)
	{
		...
		DoLine_ColorPicker(&s_HookCollTipColorResetId, ColorPickerLineSize, ColorPickerLabelSize, ColorPickerLineSpacing, &LeftView, Localize("Hook collision line tip", "Hook collision line color"), &g_Config.m_ClHookCollTipColor, ColorRGBA(1.0f, 1.0f, 0.0f, 1.0f), false, nullptr, true);
		...
		auto DoHookCollision = [this](const vec2 &Pos, const float &Length, const int &Size, const ColorRGBA &Color, const ColorRGBA &TipColor, const bool &Invert) {
			...
			if(Size > 0)
			{
				...
				Graphics()->QuadsDrawTL(&QuadItem, 1);
				if(TipColor.a > 0.0f && g_Config.m_ClHookCollAlpha)
				{
