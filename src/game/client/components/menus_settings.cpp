...
void CMenus::RenderSettingsAppearance(CUIRect MainView)
{
	...
	else if(s_CurTab == APPEARANCE_TAB_HOOK_COLLISION)
	{
		...
		DoHookCollision(TeeRenderPos, PreviewNoColl.w - LineLength - LeftMargin, color_cast<ColorRGBA>(ColorHSLA(g_Config.m_ClHookCollColorNoColl)));
		RenderTools()->RenderTee(CAnimState::GetIdle(), &OwnSkinInfo, 0, vec2(1.0f, 0.0f), TeeRenderPos);

		CUIRect NoHookTileRect;
		PreviewNoColl.VSplitRight(LineLength + LeftMargin, &PreviewNoColl, &NoHookTileRect);
		...
		PreviewColl.VSplitRight(LineLength + LeftMargin, &PreviewColl, &HookTileRect);
		...
		const vec2 DummyRenderPos = vec2(PreviewCollTee.x + PreviewCollTee.w - LineLength - 5.f, PreviewCollTee.y + PreviewCollTee.h / 2.0f);
		DoHookCollision(TeeRenderPos, PreviewCollTee.w - LineLength - LeftMargin - 15.f, color_cast<ColorRGBA>(ColorHSLA(g_Config.m_ClHookCollColorTeeColl)));
