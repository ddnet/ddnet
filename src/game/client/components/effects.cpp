...
void CEffects::AirJump(vec2 Pos, float Alpha, float Volume)
{
	...
	if(g_Config.m_SndGame)
		m_pClient->m_Sounds.PlayAt(CSounds::CHN_WORLD, SOUND_PLAYER_AIRJUMP, Volume, Pos);
}
...
void CEffects::SkidTrail(vec2 Pos, vec2 Vel, int Direction, float Alpha, float Volume)
{
	...
	if(g_Config.m_SndGame)
	{
		...
		if(Now - m_SkidSoundTimer > time_freq() / 10)
		{
			...
			m_pClient->m_Sounds.PlayAt(CSounds::CHN_WORLD, SOUND_PLAYER_SKID, Volume, Pos);
		}
	}
}
...
void CEffects::PlayerSpawn(vec2 Pos, float Alpha, float Volume)
{
	...
	if(g_Config.m_SndGame)
		m_pClient->m_Sounds.PlayAt(CSounds::CHN_WORLD, SOUND_PLAYER_SPAWN, Volume, Pos);
}
...
void CEffects::HammerHit(vec2 Pos, float Alpha, float Volume)
{
	...
	m_pClient->m_Particles.Add(CParticles::GROUP_EXPLOSIONS, &p);
	if(g_Config.m_SndGame)
		m_pClient->m_Sounds.PlayAt(CSounds::CHN_WORLD, SOUND_HAMMER_HIT, Volume, Pos);
}
