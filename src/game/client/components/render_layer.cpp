...
bool CRenderLayerGroup::DoRender(const CRenderLayerParams &Params) const
{
	if(!g_Config.m_GfxNoclip || Params.m_RenderType == CMapLayers::TYPE_FULL_DESIGN)
	{
		...
		if(m_pGroup->m_Version >= 2 && m_pGroup->m_UseClipping)
		{
			...
			int ClipX = (int)std::floor(Left * Graphics()->ScreenWidth() / ScreenWidth);
			int ClipY = (int)std::floor(Top * Graphics()->ScreenHeight() / ScreenHeight);

			Graphics()->ClipEnable(
				ClipX,
				ClipY,
				(int)std::ceil(Right * Graphics()->ScreenWidth() / ScreenWidth) - ClipX,
				(int)std::ceil(Bottom * Graphics()->ScreenHeight() / ScreenHeight) - ClipY);
		}
	}
	return true;
}
