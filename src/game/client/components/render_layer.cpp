...
static void FillTmpTile(SGraphicTile *pTmpTile, SGraphicTileTexureCoords *pTmpTex, unsigned char Flags, unsigned char Index, int x, int y, const ivec2 &Offset, int Scale)
{
	if(pTmpTex)
	{
		unsigned char TexX[]{0, 1, 1, 0};
		unsigned char TexY[]{0, 0, 1, 1};

		if(Flags & TILEFLAG_XFLIP)
			std::rotate(std::begin(TexX), std::begin(TexX) + 2, std::end(TexX));

		if(Flags & TILEFLAG_YFLIP)
			std::rotate(std::begin(TexY), std::begin(TexY) + 2, std::end(TexY));

		if(Flags & TILEFLAG_ROTATE)
		{
			std::rotate(std::begin(TexX), std::begin(TexX) + 3, std::end(TexX));
			std::rotate(std::begin(TexY), std::begin(TexY) + 3, std::end(TexY));
		}

		pTmpTex->m_TexCoordTopLeft.x = TexX[0];
		pTmpTex->m_TexCoordTopLeft.y = TexY[0];
		pTmpTex->m_TexCoordBottomLeft.x = TexX[3];
		pTmpTex->m_TexCoordBottomLeft.y = TexY[3];
		pTmpTex->m_TexCoordTopRight.x = TexX[1];
		pTmpTex->m_TexCoordTopRight.y = TexY[1];
		pTmpTex->m_TexCoordBottomRight.x = TexX[2];
		pTmpTex->m_TexCoordBottomRight.y = TexY[2];
...
void CRenderLayer::EnvelopeEvalRenderLayer(int TimeOffsetMillis, int Env, ColorRGBA &Result, size_t Channels, void *pUser)
{
	...
	CMapLayers::EnvelopeEval(TimeOffsetMillis, Env, Result, Channels, pRenderLayer->m_pMap, pRenderLayer->m_pClient, pRenderLayer->m_pGameClient);
}

bool CRenderLayerTile::STileLayerVisuals::Init(unsigned int Width, unsigned int Height)
{
	m_Width = Width;
	m_Height = Height;
	if(Width == 0 || Height == 0)
	...
	m_pTilesOfLayer = new STileLayerVisuals::STileVisual[Height * Width];
...
void CRenderLayer::OnInit(IGraphics *pGraphics, IMap *pMap, CRenderTools *pRenderTools, CMapImages *pMapImages, IClient *pClient, CGameClient *pGameClient)
{
...
const char *CRenderLayer::LoadingTitle() const
{
	return (m_pGameClient && m_pGameClient->DemoPlayer()->IsPlaying()) ? Localize("Preparing demo playback") : Localize("Loading Map");
}
...
CRenderLayerTile::STileLayerVisuals::~STileLayerVisuals()
{
	delete[] m_pTilesOfLayer;
	m_pTilesOfLayer = nullptr;
}

void CRenderLayerTile::RenderTileLayer(const ColorRGBA &Color, STileLayerVisuals *pTileLayerVisuals)
{
	STileLayerVisuals &Visuals = pTileLayerVisuals ? *pTileLayerVisuals : m_VisualTiles.value();
	...
	if(ScreenRectX1 > 0 && ScreenRectY1 > 0 && ScreenRectX0 < (int)Visuals.m_Width && ScreenRectY0 < (int)Visuals.m_Height)
	{
		...
		if(X0 <= X1)
		{
			...
			if(DrawCount != 0)
			{
				m_pGraphics->RenderTileLayer(Visuals.m_BufferContainerIndex, Color, s_vpIndexOffsets.data(), s_vDrawCounts.data(), DrawCount);
			}
		}
	}
...
{
	STileLayerVisuals &Visuals = m_VisualTiles.value();
	...
	// corners
	auto DrawCorner = [&](vec2 Offset, vec2 Scale, STileLayerVisuals::STileVisual &Visual) {
		Offset *= 32.0f;
	...
	// borders
	auto DrawBorder = [&](vec2 Offset, vec2 Scale, STileLayerVisuals::STileVisual &StartVisual, STileLayerVisuals::STileVisual &EndVisual) {
...
ColorRGBA CRenderLayerTile::GetRenderColor(const CRenderLayerParams &Params) const
{
	...
	CMapLayers::EnvelopeEval(m_pLayerTilemap->m_ColorEnvOffset, m_pLayerTilemap->m_ColorEnv, ColorEnv, 4, m_pMap, m_pClient, m_pGameClient);
...
void CRenderLayerTile::UploadTileData(std::optional<STileLayerVisuals> &VisualsOptional, int CurOverlay, bool AddAsSpeedup, bool IsGameLayer)
{
	if(!m_pGraphics->IsTileBufferingEnabled())
		return;

	if(VisualsOptional.has_value())
		VisualsOptional.reset();
	...
	const bool DoTextureCoords = GetTexture().IsValid(); // m_pLayerTilemap->m_Image >= 0 && m_pLayerTilemap->m_Image < MapLayers()->GetImages()->Num();

	// create the visual and set it in the optional, aftwards get it
	STileLayerVisuals v;
	VisualsOptional = v;
	STileLayerVisuals &Visuals = VisualsOptional.value();
	...
	for(y = 0; y < m_pLayerTilemap->m_Height; ++y)
	{
		for(x = 0; x < m_pLayerTilemap->m_Width; ++x)
		{
			...
			Visuals.m_pTilesOfLayer[y * m_pLayerTilemap->m_Width + x].SetIndexBufferByteOffset((offset_ptr32)(TilesHandledCount));

			if(AddTile(vtmpTiles, vtmpTileTexCoords, Index, Flags, x, y, DoTextureCoords, AddAsSpeedup, AngleRotate))
				Visuals.m_pTilesOfLayer[y * m_pLayerTilemap->m_Width + x].Draw(true);
...
void CRenderLayerTile::GetTileData(unsigned char *Index, unsigned char *Flags, int *AngleRotate, unsigned int x, unsigned int y, int CurOverlay) const
{
	CTile *pTiles = GetData<CTile>();
	*Index = pTiles[y * m_pLayerTilemap->m_Width + x].m_Index;
	*Flags = pTiles[y * m_pLayerTilemap->m_Width + x].m_Flags;
}
...
void CRenderLayerQuads::RenderQuadLayer(CMapItemLayerQuads *pQuadLayer, bool Force)
{
	SQuadLayerVisuals &Visuals = m_VisualQuad.value();
	if(Visuals.m_BufferContainerIndex == -1)
		return; // no visuals were created
	...
	CQuad *pQuads = (CQuad *)m_pMap->GetDataSwapped(pQuadLayer->m_Data);
	...
	for(int i = 0; i < pQuadLayer->m_NumQuads; ++i)
	{
		...
		CMapLayers::EnvelopeEval(pQuad->m_ColorEnvOffset, pQuad->m_ColorEnv, Color, 4, m_pMap, m_pClient, m_pGameClient);
		...
		if(NeedsFlush)
		{
			...
			m_pGraphics->RenderQuadLayer(Visuals.m_BufferContainerIndex, s_vQuadRenderInfo.data(), QuadsRenderCount, CurQuadOffset);
			QuadsRenderCount = 0;
			CurQuadOffset = i;
		...
		if(!IsFullyTransparent)
		{
			...
			CMapLayers::EnvelopeEval(pQuad->m_PosEnvOffset, pQuad->m_PosEnv, Position, 3, m_pMap, m_pClient, m_pGameClient);
...
void CRenderLayerQuads::Init()
{
	...
	SQuadLayerVisuals v;
	m_VisualQuad = v;
	SQuadLayerVisuals *pQLayerVisuals = &(m_VisualQuad.value());
...
void CRenderLayerQuads::Render(const CRenderLayerParams &Params)
{
	...
	if(Params.m_RenderType == CMapLayers::TYPE_BACKGROUND_FORCE || Params.m_RenderType == CMapLayers::TYPE_FULL_DESIGN)
	{
		if(g_Config.m_ClShowQuads || Params.m_RenderType == CMapLayers::TYPE_FULL_DESIGN)
		{
			...
			{
				RenderQuadLayer(m_pLayerQuads, true);
			}
		}
	}
	else
	{
		...
		{
			RenderQuadLayer(m_pLayerQuads, false);
		}
	}
}
...
void CRenderLayerEntityGame::RenderTileLayerNoTileBuffer(const ColorRGBA &Color)
{
	...
	m_pGraphics->BlendNormal();
	m_pRenderTools->RenderTileRectangle(-201, -201, m_pLayerTilemap->m_Width + 402, m_pLayerTilemap->m_Height + 402,
...
void CRenderLayerEntityTele::GetTileData(unsigned char *Index, unsigned char *Flags, int *AngleRotate, unsigned int x, unsigned int y, int CurOverlay) const
{
	CTeleTile *pTiles = GetData<CTeleTile>();
	*Index = pTiles[y * m_pLayerTilemap->m_Width + x].m_Type;
	*Flags = 0;
	if(CurOverlay == 1)
	{
		if(IsTeleTileNumberUsedAny(*Index))
			*Index = pTiles[y * m_pLayerTilemap->m_Width + x].m_Number;
		else
			*Index = 0;
	}
}
...
void CRenderLayerEntitySpeedup::GetTileData(unsigned char *Index, unsigned char *Flags, int *AngleRotate, unsigned int x, unsigned int y, int CurOverlay) const
{
	...
	*Flags = 0;
	*AngleRotate = pTiles[y * m_pLayerTilemap->m_Width + x].m_Angle;
...
void CRenderLayerEntitySwitch::GetTileData(unsigned char *Index, unsigned char *Flags, int *AngleRotate, unsigned int x, unsigned int y, int CurOverlay) const
{
	...
	*Index = pTiles[y * m_pLayerTilemap->m_Width + x].m_Type;
	if(CurOverlay == 0)
	{
		...
		*Index = pTiles[y * m_pLayerTilemap->m_Width + x].m_Delay;
}
...
void CRenderLayerEntityTune::GetTileData(unsigned char *Index, unsigned char *Flags, int *AngleRotate, unsigned int x, unsigned int y, int CurOverlay) const
{
