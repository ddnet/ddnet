...
class CMenus : public CComponent
{
	...
	int DoButton_FontIcon(CButtonContainer *pButtonContainer, const char *pText, int Checked, const CUIRect *pRect, const unsigned int Flags, int Corners = IGraphics::CORNER_ALL, bool Enabled = true);
	int DoButton_Toggle(const void *pId, int Checked, const CUIRect *pRect, bool Active, const unsigned int Flags = BUTTONFLAG_LEFT);
	int DoButton_Menu(CButtonContainer *pButtonContainer, const char *pText, int Checked, const CUIRect *pRect, const unsigned int Flags = BUTTONFLAG_LEFT, const char *pImageName = nullptr, int Corners = IGraphics::CORNER_ALL, float Rounding = 5.0f, float FontFactor = 0.0f, ColorRGBA Color = ColorRGBA(1.0f, 1.0f, 1.0f, 0.5f));
	...
	int DoButton_CheckBox_Common(const void *pId, const char *pText, const char *pBoxText, const CUIRect *pRect, const unsigned int Flags);
...
protected:
	...
	public:
		...
		{
			const int Result = str_comp_no_case(m_aName, Other.m_aName);
			return Result < 0 || (Result == 0 && str_comp_no_case(m_aClan, Other.m_aClan) < 0);
		}
	};
