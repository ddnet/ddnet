...
void CCamera::ScaleZoom(float Factor)
{
	...
	bool IsUserZoom = !m_SpectatingCameraActivated;
	...
	if(m_SpectatingCameraActivated)
	{
...
{
	m_AutoSpecCamera = true;
	m_SpecZoomTarget = std::pow(CCamera::ZOOM_STEP, g_Config.m_ClDefaultZoom - 10);
}

void CCamera::UpdateCamera()
{
	// use hardcoded smooth camera for spectating unless player explictly turn it off
	bool IsSpectatingCamera = !GameClient()->m_MultiViewActivated;
	bool UsingAutoSpecCamera = m_AutoSpecCamera && CanUseAutoSpecCamera();
	if(Client()->State() == IClient::STATE_DEMOPLAYBACK)
		IsSpectatingCamera = IsSpectatingCamera && m_pClient->m_Snap.m_SpecInfo.m_SpectatorId >= 0;
	else
		IsSpectatingCamera = IsSpectatingCamera && m_pClient->m_Snap.m_SpecInfo.m_Active && m_pClient->m_Snap.m_SpecInfo.m_SpectatorId >= 0;

	if(IsSpectatingCamera && UsingAutoSpecCamera && m_Zoom != m_pClient->m_Snap.m_SpecInfo.m_Zoom)
		ChangeZoom(m_pClient->m_Snap.m_SpecInfo.m_Zoom, 250, false);
	else if((IsSpectatingCamera && !UsingAutoSpecCamera) && m_Zoom != m_SpecZoomTarget)
		ChangeZoom(m_SpecZoomTarget, g_Config.m_ClSmoothZoomTime, false);
	else if(!IsSpectatingCamera && m_Zoom != m_UserZoomTarget)
	...
	if(m_pClient->m_Snap.m_SpecInfo.m_Active && !m_pClient->m_Snap.m_SpecInfo.m_UsePosition)
	{
		...
		m_SpectatingCameraActivated = IsSpectatingCamera;
		return;
	}

	vec2 TargetPos = IsSpectatingCamera ? m_pClient->m_CursorInfo.Target() : m_pClient->m_Controls.m_aMousePos[g_Config.m_ClDummy];
	int Smoothness = IsSpectatingCamera ? 50 : g_Config.m_ClDyncamSmoothness;
	int Stabilizing = IsSpectatingCamera ? 50 : g_Config.m_ClDyncamStabilizing;
	bool IsDyncam = IsSpectatingCamera ? true : g_Config.m_ClDyncam;
	...
	if(m_SpectatingCameraActivated != IsSpectatingCamera)
	{
		CurrentCameraOffset = m_DyncamTargetCameraOffset;
	}
...
void CCamera::OnReset()
{
	...
	m_Zooming = false;
	m_UserZoomTarget = std::pow(CCamera::ZOOM_STEP, g_Config.m_ClDefaultZoom - 10);
	m_SpecZoomTarget = std::pow(CCamera::ZOOM_STEP, g_Config.m_ClDefaultZoom - 10);
}
...
void CCamera::ConZoom(IConsole::IResult *pResult, void *pUserData)
{
	...
	bool IsSpectating = pSelf->m_SpectatingCameraActivated;

	float TargetLevel = !IsReset ? pResult->GetFloat(0) : g_Config.m_ClDefaultZoom;
	if(IsSpectating && IsReset)
		pSelf->ResetAutoSpecCamera();
	else
		pSelf->ChangeZoom(std::pow(CCamera::ZOOM_STEP, TargetLevel - 10.0f), pSelf->m_pClient->m_Snap.m_SpecInfo.m_Active && pSelf->GameClient()->m_MultiViewActivated ? g_Config.m_ClMultiViewZoomSmoothness : g_Config.m_ClSmoothZoomTime, true);
