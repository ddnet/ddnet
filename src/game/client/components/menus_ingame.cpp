	...
	if(DoButton_Menu(&s_DisconnectButton, Localize("Disconnect"), 0, &Button))
	{
			...
			GameClient()->m_Menus.m_MenusIngameTouchControls.UnsavedChanges())
		{
				...
				GameClient()->m_Menus.m_MenusIngameTouchControls.UnsavedChanges())
			{
				if(aBuf[0] == '\0')
				{
					str_copy(aBuf, Localize("There's an unsaved change in the touch controls editor, you might want to save it."));
				}
				else
				{
					str_format(aBuf, sizeof(aBuf), "%s\n\n%s", aBuf, Localize("There's an unsaved change in the touch controls editor, you might want to save it."));
				}
			}
