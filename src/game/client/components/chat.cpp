...
{
	Reset();
	Console()->Chain("cl_chat_old", [&](IConsole::IResult &Result, const IConsole::FCommandCallbackNew &FCallback) {
		FCallback(Result);
		RebuildChat();
	});
	Console()->Chain("cl_chat_size", [&](IConsole::IResult &Result, const IConsole::FCommandCallbackNew &FCallback) {
		FCallback(Result);
		EnsureCoherentWidth();
		RebuildChat();
	});
	Console()->Chain("cl_chat_width", [&](IConsole::IResult &Result, const IConsole::FCommandCallbackNew &FCallback) {
		FCallback(Result);
