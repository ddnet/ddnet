...
bool CMenusIngameTouchControls::RenderBehaviorSettingBlock(CUIRect Block)
{
	...
	if(m_EditBehaviorType != (int)EBehaviorType::BIND_TOGGLE)
	{
		...
		else if(m_EditBehaviorType == (int)EBehaviorType::PREDEFINED)
		{
			...
			const int NewPredefined = Ui()->DoDropDown(&MiddleButton, m_PredefinedBehaviorType, apPredefineds, std::size(m_BehaviorFactoriesEditor), s_ButtonPredefinedDropDownState);
...
void CMenusIngameTouchControls::SaveCachedSettingsToTarget(CTouchControls::CTouchButton *pTargetButton) const
{
	...
	else if(m_EditBehaviorType == (int)EBehaviorType::PREDEFINED)
	{
		if(m_PredefinedBehaviorType == (int)EPredefinedType::EXTRA_MENU)
			...
			pTargetButton->m_pBehavior = m_BehaviorFactoriesEditor[m_PredefinedBehaviorType].m_Factory();
	}
	else
	{
...
int CMenusIngameTouchControls::CalculatePredefinedType(const char *pType) const
{
	...
	for(IntegerType = (int)EPredefinedType::EXTRA_MENU;
		str_comp(pType, m_BehaviorFactoriesEditor[IntegerType].m_pId) != 0 && IntegerType < (int)EPredefinedType::NUM_PREDEFINEDS;
		IntegerType++)
		;
	return IntegerType;
}
...
const char **CMenusIngameTouchControls::PredefinedNames() const
{
	...
	dbg_assert(std::size(s_apPredefineds) == std::size(m_BehaviorFactoriesEditor), "Insufficient predefined names");
