...
void CMenus::RenderTouchButtonEditor(CUIRect MainView)
{
	...
	if(DoButton_Menu(&s_RemoveButton, Localize("Delete"), 0, &A))
	{
		PopupConfirm(Localize("Delete button"), Localize("Are you sure that you want to delete this button?"), Localize("Delete"), Localize("Cancel"), &CMenus::PopupConfirm_DeleteButton);
	}
	...
	if(DoButton_Menu(&s_CopyPasteButton, Localize("Duplicate"), UnsavedChanges() || Checked ? 1 : 0, &A))
	{
		if(Checked)
		{
			PopupMessage(Localize("New button already created"), Localize("A new button has already been created, please save or delete it before creating another one."), Localize("Ok"));
		}
		else if(UnsavedChanges())
		{
			PopupMessage(Localize("Unsaved changes"), Localize("Please save your changes before duplicating a button."), Localize("Ok"));
		}
		else
		{
			...
			if(FreeRect.m_X == -1)
			{
				...
				if(FreeRect.m_X == -1)
				{
					PopupMessage(Localize("No space for button"), Localize("There is not enough space available to place another button."), Localize("Ok"));
				}
				else
				{
					PopupMessage(Localize("No space for button"), Localize("There is not enough space available to place another button with this size. The button has been resized."), Localize("Ok"));
				}
			}
...
bool CMenus::RenderBehaviorSettingBlock(CUIRect Block)
{
	...
	if(m_EditBehaviorType != (int)EBehaviorType::BIND_TOGGLE)
	{
		...
		if(m_EditBehaviorType == (int)EBehaviorType::BIND)
		{
			...
			B.VSplitMid(&B, &C);
			if(DoButton_Menu(&m_vBehaviorElements[0].m_aLabelTypeRadios[0], apLabelTypes[0], NewButtonLabelType == 0 ? 1 : 0, &A, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_L))
				NewButtonLabelType = 0;
			if(DoButton_Menu(&m_vBehaviorElements[0].m_aLabelTypeRadios[1], apLabelTypes[1], NewButtonLabelType == 1 ? 1 : 0, &B, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_NONE))
				NewButtonLabelType = 1;
			if(DoButton_Menu(&m_vBehaviorElements[0].m_aLabelTypeRadios[2], apLabelTypes[2], NewButtonLabelType == 2 ? 1 : 0, &C, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_R))
	...
	else
	{
		...
		for(unsigned CommandIndex = 0; CommandIndex < m_vBehaviorElements.size(); CommandIndex++)
		{
			...
			if(s_BindToggleScrollRegion.AddRect(EditBox))
			{
				...
				B.VSplitLeft(B.w / 3.0f, &A, &B);
				B.VSplitMid(&B, &C);
				if(DoButton_Menu(&m_vBehaviorElements[CommandIndex].m_aLabelTypeRadios[0], apLabelTypes[0], NewButtonLabelType == 0 ? 1 : 0, &A, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_L))
					NewButtonLabelType = 0;
				if(DoButton_Menu(&m_vBehaviorElements[CommandIndex].m_aLabelTypeRadios[1], apLabelTypes[1], NewButtonLabelType == 1 ? 1 : 0, &B, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_NONE))
					NewButtonLabelType = 1;
				if(DoButton_Menu(&m_vBehaviorElements[CommandIndex].m_aLabelTypeRadios[0], apLabelTypes[2], NewButtonLabelType == 2 ? 1 : 0, &C, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_R))
					NewButtonLabelType = 2;
...
bool CMenus::RenderVisibilitySettingBlock(CUIRect Block)
{
	...
	for(unsigned Current = 0; Current < (unsigned)CTouchControls::EButtonVisibility::NUM_VISIBILITIES; ++Current)
	{
		...
		if(s_VisibilityScrollRegion.AddRect(EditBox))
		{
			...
			B.VSplitLeft(B.w / 3.0f, &A, &B);
			B.VSplitMid(&B, &C);
			if(DoButton_Menu(&s_avVisibilitySelector[Current][(int)EVisibilityType::INCLUDE], Localize("Included", "Touch button visibility preview"), m_aCachedVisibilities[Current] == (int)EVisibilityType::INCLUDE, &A, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_L))
			{
				m_aCachedVisibilities[Current] = (int)EVisibilityType::INCLUDE;
				SetUnsavedChanges(true);
				Changed = true;
			}
			if(DoButton_Menu(&s_avVisibilitySelector[Current][(int)EVisibilityType::EXCLUDE], Localize("Excluded", "Touch button visibility preview"), m_aCachedVisibilities[Current] == (int)EVisibilityType::EXCLUDE, &B, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_NONE))
			{
				m_aCachedVisibilities[Current] = (int)EVisibilityType::EXCLUDE;
				SetUnsavedChanges(true);
				Changed = true;
			}
			if(DoButton_Menu(&s_avVisibilitySelector[Current][(int)EVisibilityType::IGNORE], Localize("Ignore", "Touch button visibility preview"), m_aCachedVisibilities[Current] == (int)EVisibilityType::IGNORE, &C, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_R))
			{
...
void CMenus::RenderPreviewSettings(CUIRect MainView)
{
	...
	for(unsigned Current = 0; Current < (unsigned)CTouchControls::EButtonVisibility::NUM_VISIBILITIES; ++Current)
	{
		...
		if(s_VirtualVisibilityScrollRegion.AddRect(EditBox))
		{
			...
			if(Current < (unsigned)CTouchControls::EButtonVisibility::EXTRA_MENU_1)
			{
				if(DoButton_CheckBox(&m_aVisibilityIds[Current], apVisibilities[Current], aVirtualVisibilities[Current] == 1, &EditBox))
			...
			else
			{
				...
				char aBuf[64];
				str_format(aBuf, sizeof(aBuf), "%s %d", apVisibilities[(int)CTouchControls::EButtonVisibility::EXTRA_MENU_1], ExtraMenuNumber);
				if(DoButton_CheckBox(&m_aVisibilityIds[Current], aBuf, aVirtualVisibilities[Current] == 1, &EditBox))
					GameClient()->m_TouchControls.ReverseVirtualVisibilities(Current);
			}
		}
	}
...
{
	PopupMessage(Localize("No space for button"), Localize("There is not enough space available for the button. Check its visibilities and size."), Localize("Ok"));
}
...
void CMenus::SelectedButtonNotVisible()
{
	...
	m_CloseMenu = false;
	PopupConfirm(Localize("Selected button not visible"), Localize("The selected button is not visible. Do you want to deselect it or edit its visibility?"), Localize("Deselect"), Localize("Edit"), &CMenus::PopupConfirm_SelectedNotVisible);
}
...
bool CMenus::CheckCachedSettings()
{
	...
	if(!vpErrors.empty())
	{
		...
		PopupMessage(Localize("Wrong button settings"), aErrorMessage, Localize("Ok"));
		return false;
	}
	else
	{
