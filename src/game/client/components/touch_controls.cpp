...
void CTouchControls::CTouchButton::UpdateUnitFromScreenRect()
{
	const vec2 ScreenSize = m_pTouchControls->CalculateScreenSize();
	m_UnitRect.m_X = m_ScreenRect.x * BUTTON_SIZE_SCALE / ScreenSize.x;
	m_UnitRect.m_Y = m_ScreenRect.y * BUTTON_SIZE_SCALE / ScreenSize.y;
	m_UnitRect.m_W = m_ScreenRect.w * BUTTON_SIZE_SCALE / ScreenSize.x;
	m_UnitRect.m_H = m_ScreenRect.h * BUTTON_SIZE_SCALE / ScreenSize.y;
}
...
// Joystick that uses the active action. Registers/deregisters itself as the primary joystick.
CTouchControls::CJoystickActionTouchButtonBehavior::~CJoystickActionTouchButtonBehavior()
{
	if(m_pTouchControls->m_pPrimaryJoystickTouchButtonBehavior == this)
	{
		m_pTouchControls->m_pPrimaryJoystickTouchButtonBehavior = nullptr;
	}
}
