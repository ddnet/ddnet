...
static const CArgumentCompletionEntry gs_aArgumentCompletionEntries[] = {
	...
	{EArgumentCompletionType::ENVELOPE_TRIGGER, "envelope_trigger", 1},
...
static int PossibleEnvelopeTrigger(const char *pStr, IConsole::FPossibleCallback pfnCallback = IConsole::EmptyPossibleCommandCallback, void *pUser = nullptr)
{
	int Index = 0;
	for(int EnvelopeTriggerId = 0; EnvelopeTriggerId < EEnvelopeTriggerType::NUM_ENVELOPE_TRIGGERS; EnvelopeTriggerId++)
	{
...
void CGameConsole::CInstance::UpdateCompletionSuggestions()
{
	...
	if(CompletionType != EArgumentCompletionType::NONE)
	{
		...
		else if(CompletionType == EArgumentCompletionType::ENVELOPE_TRIGGER)
			PossibleEnvelopeTrigger(m_aCompletionBufferArgument, CollectPossibleCommandsCallback, &m_vpArgumentSuggestions);
