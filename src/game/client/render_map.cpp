...
void CRenderTools::RenderTeleOverlay(CTeleTile *pTele, int w, int h, float Scale, float Alpha)
{
	...
	float size = g_Config.m_ClTextEntitiesSize / 64.f;
	...
	for(int y = StartY; y < EndY; y++)
		for(int x = StartX; x < EndX; x++)
		{
			...
			if(Index && pTele[c].m_Type != TILE_TELECHECKIN && pTele[c].m_Type != TILE_TELECHECKINEVIL)
			{
				...
				UI()->TextRender()->Text(0, mx*Scale - 3.f, my*Scale, size * Scale, aBuf, -1);
		...
		for(int x = StartX; x < EndX; x++)
		{
			...
			if(Force)
			{
				...
				if(g_Config.m_ClTextEntities)
				{
					...
					UI()->TextRender()->TextColor(1.0f, 1.0f, 1.0f, Alpha);
					UI()->TextRender()->Text(0, mx*Scale, (my+0.5)*Scale, size * Scale/2.f, aBuf, -1);
					UI()->TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
					if(MaxSpeed)
					{
...
void CRenderTools::RenderTuneOverlay(CTuneTile *pTune, int w, int h, float Scale, float Alpha)
{
	...
	float size = g_Config.m_ClTextEntitiesSize * Scale / 64.f;
	...
	for(int y = StartY; y < EndY; y++)
		for(int x = StartX; x < EndX; x++)
		{
			...
			if(Index)
			{
				...
				str_format(aBuf, sizeof(aBuf), "%d", Index);
				UI()->TextRender()->TextColor(1.0f, 1.0f, 1.0f, Alpha);
				UI()->TextRender()->Text(0, mx*Scale+11.f, my*Scale+6.f, size * Scale/1.5f, aBuf, -1); // numbers shouldn't be too big and in the center of the tile
				UI()->TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
			}
		}
