...
void CRenderTools::RenderCustomEntitiesTilemap(void *pTiles, int TileSize, int IndexOffset, int FlagsOffset, int w, int h, float Scale, ColorRGBA Color, int RenderFlags) const
{
	float ScreenX0, ScreenY0, ScreenX1, ScreenY1;
	Graphics()->GetScreen(&ScreenX0, &ScreenY0, &ScreenX1, &ScreenY1);
	...
	for(int y = StartY; y < EndY; y++)
	{
		for(int x = StartX; x < EndX; x++)
		{
			...
			unsigned char Index = ((unsigned char *)pTiles)[c * TileSize + IndexOffset];
			if(Index)
			{
				unsigned char Flags = (FlagsOffset >= 0) ? ((unsigned char *)pTiles)[c * TileSize + FlagsOffset] : 0;
