...
void CRenderTools::RenderRedirectOverlay(CRedirectTile *pRedirect, int w, int h, float Scale, float Alpha) const
{
	if(!g_Config.m_ClTextEntities)
		return;
	...
	for(int y = StartY; y < EndY; y++)
	{
		for(int x = StartX; x < EndX; x++)
		{
			...
			unsigned short Index = pRedirect[c].m_Port;
			if(Index)
			{
				str_format(aBuf, sizeof(aBuf), "%u", Index);
				TextRender()->Text(mx * Scale + 11.f, my * Scale + 6.f, Size * Scale / 1.5f - 5.f, aBuf); // numbers shouldn't be too big and in the center of the tile
			}
		}
	}
...
void CRenderTools::RenderRedirectmap(CRedirectTile *pRedirect, int w, int h, float Scale, ColorRGBA Color, int RenderFlags) const
{
	float ScreenX0, ScreenY0, ScreenX1, ScreenY1;
	Graphics()->GetScreen(&ScreenX0, &ScreenY0, &ScreenX1, &ScreenY1);
	...
	for(int y = StartY; y < EndY; y++)
		for(int x = StartX; x < EndX; x++)
		{
			...
			unsigned char Index = pRedirect[c].m_Type;
			if(Index)
			{
				bool Render = false;
				if(RenderFlags & LAYERRENDERFLAG_TRANSPARENT)
