...
void CEffects::PlayerSparkle(vec2 Pos, float Alpha)
{
	if(!m_Add50hz)
	return;
...
void CEffects::PlayerTrail(vec2 Pos, float Alpha, float TimePassed)
{
	...
	CParticle p;
	p.SetDefault();
	p.m_Spr = SPRITE_PART3;
	p.m_Pos = Pos + vec2(random_float(-64, 64), random_float(-64, 64));
