...
void CPlayer::Tick()
{
	...
	if(!GameServer()->m_World.m_Paused)
	{
		...
		if(!m_pCharacter && m_DieTick+Server()->TickSpeed()*(g_Config.m_SvAutoRespawnDelay/1000) <= Server()->Tick())
			m_Spawning = true;
...
void CPlayer::SetTeam(int Team, bool DoChatMsg)
{
	...
	m_RespawnTick = Server()->Tick()+Server()->TickSpeed()*(g_Config.m_SvRespawnDelay/1000.0f);
