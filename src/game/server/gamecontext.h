...
public:
	...
	// helper functions
	void CreateDamageInd(vec2 Pos, float AngleMod, int Amount, int ClientId, CClientMask Mask = CClientMask().set());
	void CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage, int ActivatedTeam, CClientMask Mask = CClientMask().set());
	void CreateHammerHit(vec2 Pos, int ClientId, CClientMask Mask = CClientMask().set());
	void CreatePlayerSpawn(vec2 Pos, int ClientId, CClientMask Mask = CClientMask().set());
	void CreateDeath(vec2 Pos, int ClientId, CClientMask Mask = CClientMask().set());
	void CreateBirthdayEffect(vec2 Pos, int ClientId, CClientMask Mask = CClientMask().set());
	void CreateFinishEffect(vec2 Pos, int ClientId, CClientMask Mask = CClientMask().set());
	void CreateSound(vec2 Pos, int Sound, int ClientId, CClientMask Mask = CClientMask().set());
