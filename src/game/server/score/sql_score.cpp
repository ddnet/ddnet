...
bool CSqlScore::SaveTeamScoreThread(CSqlServer* pSqlServer, CSqlData *pGameData, bool HandleFailure)
{
	...
	if (HandleFailure)
	{
		...
		if(File)
		{
			...
			{
				str_format(aBuf, sizeof(aBuf), "INSERT IGNORE INTO record_teamrace(Map, Name, Timestamp, Time, ID) VALUES ('%s', '%s', CURRENT_TIMESTAMP(), '%.2f', @id);", pData->MapName(), pData->m_aNames[i], pData->m_Time);
