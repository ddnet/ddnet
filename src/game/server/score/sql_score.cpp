...
void CSqlScore::Init()
{
	...
	// create connection
	if(Connector.ConnectSqlServer())
	{
		try
		{
			char aBuf[1024];
			// get the best time
			str_format(aBuf, sizeof(aBuf), "SELECT Time FROM %s_race WHERE Map='%s' ORDER BY `Time` ASC LIMIT 0, 1;", Connector.SqlServer()->GetPrefix(), m_aMap);
			Connector.SqlServer()->executeSqlQuery(aBuf);

			if(Connector.SqlServer()->GetResults()->next())
			{
				((CGameControllerDDRace*)GameServer()->m_pController)->m_CurrentRecord = (float)Connector.SqlServer()->GetResults()->getDouble("Time");
...
void CSqlScore::LoadScoreThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			str_format(aBuf, sizeof(aBuf), "SELECT * FROM %s_race WHERE Map='%s' AND Name='%s' ORDER BY time ASC LIMIT 1;", connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_aName);
			connector.SqlServer()->executeSqlQuery(aBuf);
			if(connector.SqlServer()->GetResults()->next())
			{
				// get the best time
				float time = (float)connector.SqlServer()->GetResults()->getDouble("Time");
				...
				if(g_Config.m_SvCheckpointSave)
				{
					for(int i = 0; i < NUM_CHECKPOINTS; i++)
					{
						...
						pData->PlayerData(pData->m_ClientID)->m_aBestCpTime[i] = (float)connector.SqlServer()->GetResults()->getDouble(aColumn);
					}
				}
			}
...
void CSqlScore::SaveTeamScoreThread(void *pUser)
{
	...
	for (int retries = 0; retries < MAX_SQLSERVERS; retries++)
	{
	...
	if(connector.SqlServer())
	{
		try
		{
			...
			str_format(aBuf, sizeof(aBuf), "SELECT Name, l.ID, Time FROM ((SELECT ID FROM %s_teamrace WHERE Map = '%s' AND Name = '%s') as l) LEFT JOIN %s_teamrace as r ON l.ID = r.ID ORDER BY ID;", connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_aNames[0], connector.SqlServer()->GetPrefix());
			connector.SqlServer()->executeSqlQuery(aBuf);

			if (connector.SqlServer()->GetResults()->rowsCount() > 0)
			{
				char aID[17];
				char aID2[17];
				char aName[64];
				...
				connector.SqlServer()->GetResults()->first();
				float Time = (float)connector.SqlServer()->GetResults()->getDouble("Time");
				strcpy(aID, connector.SqlServer()->GetResults()->getString("ID").c_str());

				do
				{
					strcpy(aID2, connector.SqlServer()->GetResults()->getString("ID").c_str());
					strcpy(aName, connector.SqlServer()->GetResults()->getString("Name").c_str());
					ClearString(aName);
					if (str_comp(aID, aID2) != 0)
					{
						...
						Time = (float)connector.SqlServer()->GetResults()->getDouble("Time");
						ValidNames = true;
						Count = 0;
						strcpy(aID, aID2);
					}
			...
			if (aUpdateID[0])
			{
				str_format(aBuf, sizeof(aBuf), "UPDATE %s_teamrace SET Time='%.2f' WHERE ID = '%s';", connector.SqlServer()->GetPrefix(), pData->m_Time, aUpdateID);
				dbg_msg("SQL", aBuf);
				connector.SqlServer()->executeSql(aBuf);
			}
			else
			{
				connector.SqlServer()->executeSql("SET @id = UUID();");

				for(unsigned int i = 0; i < pData->m_Size; i++)
				{
				// if no entry found... create a new one
					str_format(aBuf, sizeof(aBuf), "INSERT IGNORE INTO %s_teamrace(Map, Name, Timestamp, Time, ID) VALUES ('%s', '%s', CURRENT_TIMESTAMP(), '%.2f', @id);", connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_aNames[i], pData->m_Time);
					dbg_msg("SQL", aBuf);
					connector.SqlServer()->executeSql(aBuf);
				}
			}
...
void CSqlScore::MapVoteThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		...
		{
			char aBuf[768];
			str_format(aBuf, sizeof(aBuf), "SELECT Map, Server FROM %s_maps WHERE Map LIKE '%s' COLLATE utf8_general_ci ORDER BY CASE WHEN Map = '%s' THEN 0 ELSE 1 END, CASE WHEN Map LIKE '%s%%' THEN 0 ELSE 1 END, LENGTH(Map), Map LIMIT 1;", connector.SqlServer()->GetPrefix(), pData->m_aMap, clearMap, clearMap);
			connector.SqlServer()->executeSqlQuery(aBuf);
			...
			if(connector.SqlServer()->GetResults()->rowsCount() != 1)
			{
			...
			else
			{
				...
				char aMap[128];
				strcpy(aMap, connector.SqlServer()->GetResults()->getString("Map").c_str());
				char aServer[32];
				strcpy(aServer, connector.SqlServer()->GetResults()->getString("Server").c_str());
...
void CSqlScore::MapInfoThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		...
		try
		{
			...
			{
				connector.SqlServer()->GetResults()->next();
				int points = (int)connector.SqlServer()->GetResults()->getInt("Points");
				int stars = (int)connector.SqlServer()->GetResults()->getInt("Stars");
				int finishes = (int)connector.SqlServer()->GetResults()->getInt("Finishes");
				int finishers = (int)connector.SqlServer()->GetResults()->getInt("Finishers");
				int average = (int)connector.SqlServer()->GetResults()->getInt("Average");
				char aMap[128];
				strcpy(aMap, connector.SqlServer()->GetResults()->getString("Map").c_str());
				char aServer[32];
				strcpy(aServer, connector.SqlServer()->GetResults()->getString("Server").c_str());
				char aMapper[128];
				strcpy(aMapper, connector.SqlServer()->GetResults()->getString("Mapper").c_str());
				int stamp = (int)connector.SqlServer()->GetResults()->getInt("Stamp");
				int ago = (int)connector.SqlServer()->GetResults()->getInt("Ago");
...
void CSqlScore::SaveScoreThread(void *pUser)
{
	...
	// Connect to database
	while (!connector.MaxTriesReached(false) && connector.ConnectSqlServer(false))
	{
		try
		{
			char aBuf[768];
			...
			if(!connector.SqlServer()->GetResults()->next())
			{
				str_format(aBuf, sizeof(aBuf), "SELECT Points FROM %s_maps WHERE Map ='%s'", connector.SqlServer()->GetPrefix(), pData->MapName());
				connector.SqlServer()->executeSqlQuery(aBuf);
				...
				if(connector.SqlServer()->GetResults()->rowsCount() == 1)
				{
					...
					str_format(aBuf, sizeof(aBuf), "INSERT INTO %s_points(Name, Points) VALUES ('%s', '%d') ON duplicate key UPDATE Name=VALUES(Name), Points=Points+VALUES(Points);", connector.SqlServer()->GetPrefix(), pData->m_aName, points);
					connector.SqlServer()->executeSql(aBuf);
				}
			}
...
void CSqlScore::ShowTeamRankThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			connector.SqlServer()->executeSql("SET @prev := NULL;");
			connector.SqlServer()->executeSql("SET @rank := 1;");
			connector.SqlServer()->executeSql("SET @pos := 0;");
			str_format(aBuf, sizeof(aBuf), "SELECT Rank, Name, Time FROM (SELECT Rank, l2.ID FROM ((SELECT ID, (@pos := @pos+1) pos, (@rank := IF(@prev = Time,@rank,@pos)) rank, (@prev := Time) Time FROM (SELECT ID, Time FROM %s_teamrace WHERE Map = '%s' GROUP BY ID ORDER BY Time) as ll) as l2) LEFT JOIN %s_teamrace as r2 ON l2.ID = r2.ID WHERE Map = '%s' AND Name = '%s' ORDER BY Rank LIMIT 1) as l LEFT JOIN %s_teamrace as r ON l.ID = r.ID ORDER BY Name;", connector.SqlServer()->GetPrefix(), pData->MapName(), connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_aName, connector.SqlServer()->GetPrefix());

			connector.SqlServer()->executeSqlQuery(aBuf);

			int Rows = connector.SqlServer()->GetResults()->rowsCount();
			...
			else
			{
				...
				float Time = (float)connector.SqlServer()->GetResults()->getDouble("Time");
				int Rank = (int)connector.SqlServer()->GetResults()->getInt("Rank");

				for(int Row = 0; Row < Rows; Row++)
				{
					strcat(aNames, connector.SqlServer()->GetResults()->getString("Name").c_str());
					connector.SqlServer()->GetResults()->next();
		...
		try
		{
			...
			if (Rows >= 1) {
				...
				for(int Row = 0; Row < Rows; Row++)
				{
					...
					Time = (float)connector.SqlServer()->GetResults()->getDouble("Time");
					Rank = (float)connector.SqlServer()->GetResults()->getInt("rank");
...
void CSqlScore::ShowRankThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			connector.SqlServer()->executeSql("SET @rank := 1;");
			connector.SqlServer()->executeSql("SET @pos := 0;");
			str_format(aBuf, sizeof(aBuf), "SELECT Rank, Name, Time FROM (SELECT Name, (@pos := @pos+1) pos, (@rank := IF(@prev = Time,@rank, @pos)) rank, (@prev := Time) Time FROM (SELECT Name, min(Time) as Time FROM %s_race WHERE Map = '%s' GROUP BY Name ORDER BY `Time` ASC) as a) as b WHERE Name = '%s';", connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_aName);
			...
			else
			{
				...
				if(g_Config.m_SvHideScore)
					...
					str_format(aBuf, sizeof(aBuf), "%d. %s Time: %02d:%05.2f, requested by %s", Rank, connector.SqlServer()->GetResults()->getString("Name").c_str(), (int)(Time/60), Time-((int)Time/60*60), pData->m_aRequestingPlayer);
...
void CSqlScore::ShowTop5Thread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			connector.SqlServer()->executeSql("SET @rank := 1;");
			connector.SqlServer()->executeSql("SET @pos := 0;");
			str_format(aBuf, sizeof(aBuf), "SELECT Name, Time, rank FROM (SELECT Name, (@pos := @pos+1) pos, (@rank := IF(@prev = Time,@rank, @pos)) rank, (@prev := Time) Time FROM (SELECT Name, min(Time) as Time FROM %s_race WHERE Map = '%s' GROUP BY Name ORDER BY `Time` ASC) as a) as b LIMIT %d, 5;", connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_Num-1);
			connector.SqlServer()->executeSqlQuery(aBuf);
			...
			while(connector.SqlServer()->GetResults()->next())
			{
				Time = (float)connector.SqlServer()->GetResults()->getDouble("Time");
				Rank = (float)connector.SqlServer()->GetResults()->getInt("rank");
				str_format(aBuf, sizeof(aBuf), "%d. %s Time: %02d:%05.2f", Rank, connector.SqlServer()->GetResults()->getString("Name").c_str(), (int)(Time/60), Time-((int)Time/60*60));
				pData->GameServer()->SendChatTarget(pData->m_ClientID, aBuf);
				//Rank++;
			}
			pData->GameServer()->SendChatTarget(pData->m_ClientID, "-------------------------------");
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			if(pData->m_Search) // last 5 times of a player
				str_format(aBuf, sizeof(aBuf), "SELECT Time, UNIX_TIMESTAMP(CURRENT_TIMESTAMP)-UNIX_TIMESTAMP(Timestamp) as Ago, UNIX_TIMESTAMP(Timestamp) as Stamp FROM %s_race WHERE Map = '%s' AND Name = '%s' ORDER BY Ago ASC LIMIT %d, 5;", connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_aName, pData->m_Num-1);
			else// last 5 times of server
				str_format(aBuf, sizeof(aBuf), "SELECT Name, Time, UNIX_TIMESTAMP(CURRENT_TIMESTAMP)-UNIX_TIMESTAMP(Timestamp) as Ago, UNIX_TIMESTAMP(Timestamp) as Stamp FROM %s_race WHERE Map = '%s' ORDER BY Ago ASC LIMIT %d, 5;", connector.SqlServer()->GetPrefix(), pData->MapName(), pData->m_Num-1);
			...
			str_format(aBuf, sizeof(aBuf), "------------ Last Times No %d - %d ------------",pData->m_Num,pData->m_Num + connector.SqlServer()->GetResults()->rowsCount() - 1);
			pData->GameServer()->SendChatTarget(pData->m_ClientID, aBuf);
			...
			{
				char pAgoString[40] = "\0";
				pSince = (int)connector.SqlServer()->GetResults()->getInt("Ago");
				pStamp = (int)connector.SqlServer()->GetResults()->getInt("Stamp");
				pTime = (float)connector.SqlServer()->GetResults()->getDouble("Time");
				...
				{
					if(pStamp == 0) // stamp is 00:00:00 cause it's an old entry from old times where there where no stamps yet
						str_format(aBuf, sizeof(aBuf), "%s, %02d:%05.02f s, don't know when", connector.SqlServer()->GetResults()->getString("Name").c_str(), (int)(pTime/60), pTime-((int)pTime/60*60));
					else
						str_format(aBuf, sizeof(aBuf), "%s, %s ago, %02d:%05.02f s", connector.SqlServer()->GetResults()->getString("Name").c_str(), pAgoString, (int)(pTime/60), pTime-((int)pTime/60*60));
				}
				pData->GameServer()->SendChatTarget(pData->m_ClientID, aBuf);
			}
			pData->GameServer()->SendChatTarget(pData->m_ClientID, "----------------------------------------------------");
...
void CSqlScore::ShowPointsThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			char aBuf[512];
			str_format(aBuf, sizeof(aBuf), "select Rank, Name, Points from (select (@pos := @pos+1) pos, (@rank := IF(@prev = Points,@rank,@pos)) Rank, Points, Name from (select (@prev := Points) Points, Name from %s_points order by Points desc) as ll) as l where Name = '%s';", connector.SqlServer()->GetPrefix(), pData->m_aName);
			connector.SqlServer()->executeSqlQuery(aBuf);
			...
			{
				connector.SqlServer()->GetResults()->next();
				int count = (int)connector.SqlServer()->GetResults()->getInt("Points");
				int rank = (int)connector.SqlServer()->GetResults()->getInt("rank");
				str_format(aBuf, sizeof(aBuf), "%d. %s Points: %d, requested by %s", rank, connector.SqlServer()->GetResults()->getString("Name").c_str(), count, pData->m_aRequestingPlayer);
				pData->GameServer()->SendChat(-1, CGameContext::CHAT_ALL, aBuf, pData->m_ClientID);
			}
...
void CSqlScore::ShowTopPointsThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			connector.SqlServer()->executeSql("SET @rank := 1;");
			connector.SqlServer()->executeSql("SET @pos := 0;");
			str_format(aBuf, sizeof(aBuf), "select Rank, Name, Points from (select (@pos := @pos+1) pos, (@rank := IF(@prev = Points,@rank,@pos)) Rank, Points, Name from (select (@prev := Points) Points, Name from %s_points order by Points desc) as ll) as l LIMIT %d, 5;", connector.SqlServer()->GetPrefix(), pData->m_Num-1);
			...
			while(connector.SqlServer()->GetResults()->next())
			{
				str_format(aBuf, sizeof(aBuf), "%d. %s Points: %d", connector.SqlServer()->GetResults()->getInt("rank"), connector.SqlServer()->GetResults()->getString("Name").c_str(), connector.SqlServer()->GetResults()->getInt("Points"));
				pData->GameServer()->SendChatTarget(pData->m_ClientID, aBuf);
			}
			pData->GameServer()->SendChatTarget(pData->m_ClientID, "-------------------------------");
...
void CSqlScore::RandomMapThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			char aBuf[512];
			if(pData->m_Num)
				str_format(aBuf, sizeof(aBuf), "select * from %s_maps where Server = \"%s\" and Stars = \"%d\" order by RAND() limit 1;", connector.SqlServer()->GetPrefix(), g_Config.m_SvServerType, pData->m_Num);
			else
				str_format(aBuf, sizeof(aBuf), "select * from %s_maps where Server = \"%s\" order by RAND() limit 1;", connector.SqlServer()->GetPrefix(), g_Config.m_SvServerType);
			connector.SqlServer()->executeSqlQuery(aBuf);
...
void CSqlScore::RandomUnfinishedMapThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		try
		{
			...
			char aBuf[512];
			if(pData->m_Num)
				str_format(aBuf, sizeof(aBuf), "select * from %s_maps where Server = \"%s\" and Stars = \"%d\" and not exists (select * from %s_race where Name = \"%s\" and %s_race.Map = %s_maps.Map) order by RAND() limit 1;", connector.SqlServer()->GetPrefix(), g_Config.m_SvServerType, pData->m_Num, connector.SqlServer()->GetPrefix(), pData->m_aName, connector.SqlServer()->GetPrefix(), connector.SqlServer()->GetPrefix());
			else
				str_format(aBuf, sizeof(aBuf), "select * from %s_maps where Server = \"%s\" and not exists (select * from %s_race where Name = \"%s\" and %s_race.Map = %s_maps.Map) order by RAND() limit 1;", connector.SqlServer()->GetPrefix(), g_Config.m_SvServerType, connector.SqlServer()->GetPrefix(), pData->m_aName, connector.SqlServer()->GetPrefix(), connector.SqlServer()->GetPrefix());
			connector.SqlServer()->executeSqlQuery(aBuf);
...
void CSqlScore::SaveTeamThread(void *pUser)
{
	...
	// Connect to database
	if(!Num && connector.ConnectSqlServer())
	{
		try
		{
			char aBuf[512];
			str_format(aBuf, sizeof(aBuf), "select Savegame from %s_saves where Code = '%s' and Map = '%s';",  connector.SqlServer()->GetPrefix(), pData->m_Code, Map);
			connector.SqlServer()->executeSqlQuery(aBuf);

			if (connector.SqlServer()->GetResults()->rowsCount() == 0)
			{
				char aBuf[65536];
				str_format(aBuf, sizeof(aBuf), "INSERT IGNORE INTO %s_saves(Savegame, Map, Code, Timestamp, Server) VALUES ('%s', '%s', '%s', CURRENT_TIMESTAMP(), '%s')",  connector.SqlServer()->GetPrefix(), TeamString, Map, pData->m_Code, pData->m_Server);
				dbg_msg("SQL", aBuf);
				connector.SqlServer()->executeSql(aBuf);
...
void CSqlScore::LoadTeamThread(void *pUser)
{
	...
	if(connector.ConnectSqlServer())
	{
		...
		{
			char aBuf[768];
			str_format(aBuf, sizeof(aBuf), "select Savegame, Server, UNIX_TIMESTAMP(CURRENT_TIMESTAMP)-UNIX_TIMESTAMP(Timestamp) as Ago from %s_saves where Code = '%s' and Map = '%s';",  connector.SqlServer()->GetPrefix(), pData->m_Code, Map);
			connector.SqlServer()->executeSqlQuery(aBuf);
			...
			if (connector.SqlServer()->GetResults()->rowsCount() > 0)
			{
				connector.SqlServer()->GetResults()->first();
				char ServerName[5];
				str_copy(ServerName, connector.SqlServer()->GetResults()->getString("Server").c_str(), sizeof(ServerName));
				...
				connector.SqlServer()->GetResults()->getInt("Ago");
				int since = (int)connector.SqlServer()->GetResults()->getInt("Ago");
				...
				Num = SavedTeam->LoadString(connector.SqlServer()->GetResults()->getString("Savegame").c_str());
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							char aBuf[512];
							str_format(aBuf, sizeof(aBuf), "DELETE from %s_saves where Code='%s' and Map='%s';", connector.SqlServer()->GetPrefix(), pData->m_Code, Map);
							connector.SqlServer()->executeSql(aBuf);
						}
					}
				}
