...
void CGameContext::ConHelp(IConsole::IResult *pResult, void *pUserData)
{
	...
	else
	{
		...
		if(pCmdInfo)
		{
			...
			if(pCmdInfo->m_pHelp)
				log_info("chatresp", "%s", pCmdInfo->m_pHelp);
		}
		else
		{
			char aBuf[256];
...
void CGameContext::ConSettings(IConsole::IResult *pResult, void *pUserData)
{
	...
	else
	{
		...
		{
			log_info("chatresp", g_Config.m_SvSlashMe ?
...
void CGameContext::ConSwap(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		log_info("chatresp", "Join a team to use swap feature, which means you can swap positions with each other.");
		return;
	}
...
void CGameContext::AttemptJoinTeam(int ClientId, int Team)
{
	...
	{
		log_info("chatresp", "You can't change teams while you are dead/a spectator.");
	}
	else
	{
...
void CGameContext::ConMe(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(g_Config.m_SvSlashMe)
		...
		log_info("chatresp", "/me is disabled on this server");
}
