...
void CGameContext::ConPractice(IConsole::IResult *pResult, void *pUserData)
{
	...
	CGameTeams &Teams = pSelf->m_pController->m_Teams;
...
void CGameContext::ConLockTeam(IConsole::IResult *pResult, void *pUserData)
{
	...
	int Team = pSelf->m_pController->m_Teams.m_Core.Team(pResult->m_ClientID);

	bool Lock = pSelf->m_pController->m_Teams.TeamLocked(Team);
	...
	{
		pSelf->m_pController->m_Teams.SetTeamLock(Team, true);
	...
	else
	{
		...
		else
		{
			str_format(
				...
				sizeof(aBuf),
				"You are in team %d",
				pSelf->m_pController->m_Teams.m_Core.Team(pResult->m_ClientID));
			pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chatresp",
				aBuf);
		}
	}
