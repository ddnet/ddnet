...
static void ToggleSpecPause(IConsole::IResult *pResult, void *pUserData, int PauseType)
{
	...
	//yirou: block ANY self-exit from spec/pause when relay forced or toggle disabled
	bool IsInSpecOrPause = (PauseState < 0); // negative means in spec or paused
	bool IsExiting = (PauseType == CPlayer::PAUSE_NONE) || 
	                 (IsInSpecOrPause && PauseType != CPlayer::PAUSE_NONE); // switching types also exits current state
	if(IsInSpecOrPause && IsExiting && (pPlayer->IsRelayForcedSpec() || !pPlayer->CanToggleSpec()))
	{
		pSelf->SendChatTarget(pResult->m_ClientId, "You cannot exit spec/pause right now");
		return;
	}
	//yirou: block entering spec when toggle is disabled
	if(PauseState == CPlayer::PAUSE_NONE && !pPlayer->CanToggleSpec())
	{
		pSelf->SendChatTarget(pResult->m_ClientId, "You cannot toggle spec right now");
		return;
	}
	if(PauseState > 0)
	{
...
static void ToggleSpecPauseVoted(IConsole::IResult *pResult, void *pUserData, int PauseType)
{
	...
	int PauseState = pPlayer->IsPaused();
	//yirou: block ANY self-exit from spec/pause when relay forced or toggle disabled
	if(PauseState < 0 && (pPlayer->IsRelayForcedSpec() || !pPlayer->CanToggleSpec()))
	{
		return;
	}
...
void CGameContext::ConToggleSpec(IConsole::IResult *pResult, void *pUserData)
{
	...
	//yirou: block self-exit from relay forced spec or when toggle is disabled
	if(pPlayer->IsRelayForcedSpec() || !pPlayer->CanToggleSpec())
	{
		pSelf->SendChatTarget(pResult->m_ClientId, "You cannot toggle spec right now");
		return;
	}
...
void CGameContext::ConStartRelay(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(!pChr)
	{
		pSelf->SendChatTarget(pResult->m_ClientId, "You need to be alive to start the relay");
		return;
	}
...
//yirou
void CGameContext::ConSetRelayOrder(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientID = pResult->m_ClientId;
	...
	if(pResult->NumArguments() == 0)
	{
		pSelf->SendChatTarget(ClientID, "Usage: /relayset <order>");
		return;
	}

	int Order = pResult->GetInteger(0);
	CGameTeams &Teams = pSelf->m_pController->Teams();
	Teams.SetRelayRunner(Team, Order, ClientID);
	pSelf->SendChatTarget(ClientID, "成功");
	Teams.SendRelayOrder(Team);
}

//yirou
void CGameContext::ConRelayBack(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientID = pResult->m_ClientId;
...
//yirou
void CGameContext::ConRelayPause(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientID = pResult->m_ClientId;
