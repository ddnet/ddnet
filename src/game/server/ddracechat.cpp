...
void CGameContext::AttemptJoinTeam(int ClientId, int Team)
{
	...
	else
	{
		...
		else if(Team > 0 && Team < NUM_DDRACE_TEAMS && m_pController->Teams().TeamLocked(Team) && !m_pController->Teams().IsInvited(Team, ClientId))
		{
			Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chatresp",
				g_Config.m_SvInvite ?
		...
		else if(Team > 0 && Team < NUM_DDRACE_TEAMS && m_pController->Teams().Count(Team) >= g_Config.m_SvMaxTeamSize && !m_pController->Teams().TeamFlock(Team) && !m_pController->Teams().IsPractice(Team))
		{
			char aBuf[512];
