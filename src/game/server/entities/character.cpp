...
void CCharacter::HandleNinja()
{
	...
	if(m_Core.m_Ninja.m_CurrentMoveTime > 0)
	{
		...
		// check if we Hit anything along the way
		{
			...
			Num = GameServer()->m_World.FindEntities(OldPos, Radius, apTargetEnts, MAX_TARGET_SWITCHES, CGameWorld::ENTTYPE_TARGETSWITCH);
			...
			for(int i = 0; i < Num; ++i)
			{
				...
				dbg_assert(m_NumTargetSwitchesHit < MAX_TARGET_SWITCHES, "m_aHitTargetSwitches overflow");
