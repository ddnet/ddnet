...
void CGameContext::ConTeleport(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(!CheckClientId(pResult->m_ClientId))
		return;
	int Tele = pResult->NumArguments() == 2 ? pResult->GetVictim(0) : pResult->m_ClientId;
	int TeleTo = pResult->NumArguments() ? pResult->GetVictim(pResult->NumArguments() - 1) : pResult->m_ClientId;
