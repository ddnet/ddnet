...
{
	for(int i = 0; i < MAX_CLIENTS; i++)
		if(m_pController->m_Teams.m_Core.Team(i) == Team)
...
void CGameContext::OnPreTickTeehistorian()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(m_apPlayers[i] != nullptr)
			m_TeeHistorian.RecordPlayerTeam(i, m_pController->m_Teams.m_Core.Team(i));
		else
			m_TeeHistorian.RecordPlayerTeam(i, 0);
	}
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		m_TeeHistorian.RecordTeamPractice(i, m_pController->m_Teams.IsPractice(i));
	}
}
...
void CGameContext::OnSayNetMessage(const CNetMsg_Cl_Say *pMsg, int ClientID, const CUnpacker *pUnpacker)
{
	...
	int GameTeam = (m_pController)->m_Teams.m_Core.Team(pPlayer->GetCID());
	if(Team)
...
{
	return m_pController->m_Teams.m_Core.Team(ClientID);
}
