...
{
	dbg_assert(in_range(Type, 0, NUM_IMPORTANTALERTTYPES - 1), "SendImportantAlert Type invalid: %d", Type);
	...
	const auto &&SendImportantAlertToClient = [&](int ClientId) {
		...
		else
		{
			...
			switch(Type)
			{
			case IMPORTANTALERTTYPE_SERVER:
				str_append(aBroadcastText, "SERVER ALERT\n\n");
				break;
			case IMPORTANTALERTTYPE_MODERATOR:
				str_append(aBroadcastText, "MODERATOR ALERT\n\n");
				log_info("moderator_alert", "Notice: player uses an old client version and may not see important alerts: %s (ID %d)", Server()->ClientName(ClientId), ClientId);
				break;
			}
...
void CGameContext::ConServerAlert(IConsole::IResult *pResult, void *pUserData)
{
	...
	pSelf->SendImportantAlert(IMPORTANTALERTTYPE_SERVER, aBuf, -1);
}
...
void CGameContext::ConModeratorAlert(IConsole::IResult *pResult, void *pUserData)
{
	...
	pSelf->SendImportantAlert(IMPORTANTALERTTYPE_MODERATOR, aBuf, Victim);
}
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("mod_alert", "v[id] r[message]", CFGFLAG_SERVER, ConModeratorAlert, this, "Send a moderator alert message to player");
