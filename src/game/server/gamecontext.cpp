...
void CGameContext::ConRandomMap(IConsole::IResult *pResult, void *pUserData)
{
	...
	int MaxStars = pResult->NumArguments() > 1 ? pResult->GetInteger(1) : -1;

	if(MaxStars != -1 && Stars > MaxStars)
	{
		std::swap(Stars, MaxStars);
	}

	pSelf->m_pScore->RandomMap(ClientId, Stars, MaxStars);
}
...
void CGameContext::ConRandomUnfinishedMap(IConsole::IResult *pResult, void *pUserData)
{
	...
	pSelf->m_pScore->RandomUnfinishedMap(ClientId, Stars, MaxStars);
}
